package com.taomee.seer2.core.scene
{
   ...
   public class SceneManager
   {
      ...
      private static function loadMap(param1:SwitchData) : void
      {
         ...
         _loc2_.addEventListener("loadError",onLoadError);
         _loc2_.addEventListener("loadComplete",onLoadComplete);
         dispatchEvent("switchStart");
      }
      ...
      private static function onLoadError(param1:SceneEvent) : void
      {
         ...
         dispatchEvent("switchError");
         MapPanelProtocolPanel.instance().addLog(1," 跳转出错了···");
      }
      ...
      {
         param1.removeEventListener("loadError",onLoadError);
         param1.removeEventListener("loadComplete",onLoadComplete);
         param1.dispose();
      }
      ...
      private static function processLoaderComplete(param1:BaseMapLoader) : void
      {
         ...
         recordPrevInfo();
         if(_switchData.switchType == "swtichScene")
         {
            if(_currentScene)
            {
         ...
         _loc2_.addEventListener("mapComplete",onMapComplete);
         _currentScene.updateMap(_loc2_);
      }
      
      private static function recordPrevInfo() : void
      {
         _preSceneType = _currentScene ? _currentScene.type : 0;
      ...
      private static function onMapComplete(param1:SceneEvent) : void
      {
         ...
         _loc2_.removeEventListener("mapComplete",onMapComplete);
         processSwitchComplete();
         if(null != _callBack)
         {
            _callBack();
         ...
         dispatchEvent("switchComplete");
         MapPanelProtocolPanel.instance().addLog(1," name=\"" + _loc2_.name + "\"");
      }
      ...
      private static function processSwitchComplete() : void
      {
         _isSwitching = false;
         _switchData = null;
         if(currentSceneType == 2 || currentSceneType == 6 || currentSceneType == 10 || currentSceneType == 11 || currentSceneType == 13 || currentSceneType == 12 || currentSceneType == 14 || currentSceneType == 15 || currentSceneType == 16)
         {
      ...
      {
         return _currentScene ? _currentScene.type : 0;
      }
