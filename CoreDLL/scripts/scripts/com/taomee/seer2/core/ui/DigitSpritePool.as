package com.taomee.seer2.core.ui
{
   ...
   internal class DigitSpritePool
   {
      ...
      public function recycle() : void
      {
         ...
         var _loc3_:Vector.<Sprite> = null;
         var _loc2_:int = int(this._activeVec.length);
         var _loc1_:int = _loc2_ - 1;
         while(_loc1_ >= 0)
         {
            _loc4_ = this._activeVec[_loc1_];
            if(_loc4_.stage == null)
            {
               _loc3_ = this.getDigitSpriteVec(int(_loc4_.name));
               _loc3_.push(_loc4_);
               this._activeVec.splice(_loc1_,1);
            }
            _loc1_--;
         }
      }
