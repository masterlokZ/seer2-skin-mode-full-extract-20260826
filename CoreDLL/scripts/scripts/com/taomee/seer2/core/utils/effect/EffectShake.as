package com.taomee.seer2.core.utils.effect
{
   ...
   public class EffectShake
   {
      ...
      private static function shake(param1:Number) : void
      {
         ...
         var _loc4_:Array = _shake[param1];
         var _loc6_:DisplayObject = _loc4_[0];
         var _loc5_:Array = _loc4_[1];
         var _loc3_:Array = _loc4_[2];
         if(!isNaN(_loc5_[2]) && _loc5_[2] > 0)
         {
            if(_loc6_.x != _loc3_[0])
            {
               _loc6_.x = _loc3_[0];
            }
            else
            {
               _loc6_.x = dec(-1,1) * _loc5_[0] + _loc3_[0];
            }
            --_loc5_[2];
            _loc2_ = true;
         }
         if(!isNaN(_loc5_[3]) && _loc5_[3] > 0)
         {
            if(_loc6_.y != _loc3_[1])
            {
               _loc6_.y = _loc3_[1];
            }
            else
            {
               _loc6_.y = dec(-1,1) * _loc5_[1] + _loc3_[1];
            }
            --_loc5_[3];
            _loc2_ = true;
         }
      ...
      public static function arrayIndex(param1:Array, param2:DisplayObject, param3:uint) : Number
      {
         ...
         {
            if(param1[_loc4_][param3] == param2)
            {
               _loc5_ = int(_loc4_);
               break;
            }
