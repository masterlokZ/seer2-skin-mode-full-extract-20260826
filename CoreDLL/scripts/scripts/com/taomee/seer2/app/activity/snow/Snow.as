package com.taomee.seer2.app.activity.snow
{
   ...
   public class Snow extends Sprite
   {
      ...
      public function Snow()
      {
         ...
         this.snowCount = 30;
         this.CreateSnows();
         addEventListener("enterFrame",this.moveSnow);
      }
      ...
      private function CreateSnows() : void
      {
         ...
         {
            _loc1_ = this.CreateOneSnow();
            addChild(_loc1_);
            this.snowSp.push(_loc1_);
            _loc1_.x = 960 * Math.random() - 100;
            _loc1_.y = 560 * Math.random();
            _loc1_.name = "snow" + _loc2_;
            _loc2_++;
         }
      }
      ...
      public function dispose() : void
      {
         ...
         var _loc1_:Shape = null;
         for each(_loc2_ in this.snowSp)
         {
            DisplayUtil.removeForParent(_loc2_);
         }
         for each(_loc1_ in this.snowArr)
         {
            DisplayUtil.removeForParent(_loc1_);
         }
         this.snowArr = null;
         this.snowSp = null;
         removeEventListener("enterFrame",this.moveSnow);
      }
   }
}
