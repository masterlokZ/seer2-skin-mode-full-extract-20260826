package com.taomee.seer2.app.activity.onlineReward
{
   ...
   public class MakeWishAnimationManager
   {
      ...
      {
         if(Boolean(SceneManager.active) && (SceneManager.active.type == 2 || DreamManager.isDreamMap() || SceneManager.active.type == 3))
         {
            return;
         }
         if(_mc)
         ...
         {
            QueueLoader.load(URLUtil.getActivityAnimation("makeWishAnimation"),"swf",onResLoaded);
         }
      }
      ...
      private static function showMC() : void
      {
         ...
         _btn.addEventListener("click",onBirth);
         updateTime();
      }
      ...
      {
         _time.visible = false;
         ServerBufferManager.getServerBuffer(100002,onGetTime,false);
      }
