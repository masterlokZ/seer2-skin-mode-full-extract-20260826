package com.taomee.seer2.app.activity.onlineReward
{
   ...
   public class NonoNews
   {
      ...
      {
         this._gotoMapBtn.addEventListener("click",this.onClick);
         this._timer.addEventListener("timer",this.onMove);
         this._timer.start();
         var _loc1_:int = 0;
         while(_loc1_ < 6)
         {
            (this._rewardPanel["page_" + _loc1_] as MovieClip).buttonMode = true;
            (this._rewardPanel["page_" + _loc1_] as MovieClip).addEventListener("rollOver",this.onOver);
            (this._rewardPanel["page_" + _loc1_] as MovieClip).addEventListener("rollOut",this.onOut);
            _loc1_++;
         }
      }
      ...
      private function onClick(param1:MouseEvent) : void
      {
         ...
         StatisticsManager.sendNovice("0x10033002");
         StatisticsManager.sendNovice("0x10033648");
         ...
         {
            _loc3_ = this._nonoNewsInfo.transport.split("|");
            if(_loc3_.length < 2)
            {
               ModuleManager.toggleModule(URLUtil.getAppModule(_loc3_[0]),"正在打开面板...");
            }
            else
            {
               ...
               ModuleManager.toggleModule(URLUtil.getAppModule(_loc3_[0]),"正在打开面板...",_loc4_);
            }
         }
         this.dispose();
      }
      ...
      public function dispose() : void
      {
         ...
         {
            this._timer.stop();
            this._timer.removeEventListener("timer",this.onMove);
         }
         (this._rewardPanel["page_" + this._currIndex] as MovieClip).gotoAndStop("normal");
         this._currIndex = 0;
         (this._rewardPanel["page_" + this._currIndex] as MovieClip).gotoAndStop("select");
