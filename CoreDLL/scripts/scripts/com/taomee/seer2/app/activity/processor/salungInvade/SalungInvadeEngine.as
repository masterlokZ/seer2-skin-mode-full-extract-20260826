package com.taomee.seer2.app.activity.processor.salungInvade
{
   ...
   public class SalungInvadeEngine
   {
      ...
      {
         SceneManager.addEventListener("switchComplete",this.mapInitComplete);
      }
      ...
      private function prepareGame() : void
      {
         ...
         var _loc2_:uint = this.getRandom();
         _loc2_ = uint(MAP_TASK[this._randomMapName][_loc2_]);
         if(_loc2_ == 0)
         {
            _loc1_ = new SalungShootEngine(this._mapId);
         }
         else if(_loc2_ == 1)
         {
            _loc1_ = new SalungHostageTarget(this._mapId);
         }
         else if(_loc2_ == 2)
         {
            _loc1_ = new SalungBoxing(this._mapId);
         }
         else if(_loc2_ == 3)
         {
            _loc1_ = new CloudSoldierController(this._mapId);
         }
         else if(_loc2_ == 4)
         {
            _loc1_ = new KaisaFight(this._mapId);
         }
         else if(_loc2_ == 5)
         {
            _loc1_ = new DoomBoltSoldier(this._mapId);
         }
