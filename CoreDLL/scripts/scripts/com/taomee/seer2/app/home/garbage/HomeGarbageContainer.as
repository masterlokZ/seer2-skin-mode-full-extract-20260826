package com.taomee.seer2.app.home.garbage
{
   ...
   public class HomeGarbageContainer
   {
      ...
      {
         var _loc4_:HomeGarbage = null;
         this._garbageVec = new Vector.<HomeGarbage>();
         HomeGarbageHelper.prepare();
         var _loc6_:int = param1.homeInfo.garbageCount;
         var _loc5_:int = 0;
         while(_loc5_ < _loc6_)
         {
            _loc4_ = new HomeGarbage(HomeGarbageHelper.getGarbageData(),param3);
            param2.content.addChild(_loc4_);
            this._garbageVec.push(_loc4_);
            _loc4_.addEventListener("open",this.onGarbageStart);
            _loc4_.addEventListener("complete",this.onGarbageComplete);
            _loc5_++;
         }
      }
      ...
      public function clearGarbage() : void
      {
         ...
         {
            _loc1_.removeEventListener("open",this.onGarbageStart);
            _loc1_.removeEventListener("complete",this.onGarbageComplete);
            _loc1_.dispose();
         }
