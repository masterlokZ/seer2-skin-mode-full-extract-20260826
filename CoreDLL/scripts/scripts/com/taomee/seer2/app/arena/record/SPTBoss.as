   ...
   public class SPTBoss implements IFightRequest
   {
      ...
      public function startFight(param1:Object, param2:ButtonPanelData, param3:Number = -1) : void
      {
         ...
         onSuccess = function():void
         {
            ...
            _loc4_.writeUnsignedInt(bossId);
            ...
            {
               _loc3_ = int(params.length);
               _loc4_.writeUnsignedInt(_loc3_);
               _loc6_ = 0;
               while(_loc6_ < _loc3_)
               {
                  _loc4_.writeUnsignedInt(params[_loc6_]);
                  _loc6_++;
               }
            }
            ...
            {
               _loc4_.writeByte(2);
               _loc5_ = initData.petIds;
               _loc2_ = _loc5_.length;
               _loc4_.writeUnsignedInt(_loc2_);
               _loc1_ = 0;
               while(_loc1_ < _loc2_)
               {
                  _loc4_.writeUnsignedInt(_loc5_[_loc1_]);
                  _loc1_++;
               }
            }
            Connection.send(CommandSet.BOSS_START_FIGHT_1046,_loc4_);
            MapPanelProtocolPanel.instance().addLog(7,"\n战斗协议：发起战斗成功 boosID=" + bossId);
         ...
         onFail = function():void
         {
            ...
            FightManager.dispatchEvent(new FightStartEvent("START_ERROR"));
