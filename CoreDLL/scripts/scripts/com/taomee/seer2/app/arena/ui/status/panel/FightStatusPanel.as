package com.taomee.seer2.app.arena.ui.status.panel
{
   ...
   public class FightStatusPanel extends Sprite
   {
      ...
      private var _tm:Timer = new Timer(2000,1);
      ...
      public function FightStatusPanel()
      {
         ...
         this.createBar(1);
         this.createBar(2);
         this.createBuffIconBar(1);
         this.createBuffIconBar(2);
         this.createSkillBubble(1);
         this.createSkillBubble(2);
         this.createCapsuleBar(1);
         this.createCapsuleBar(2);
         this.createAngerBar(1);
         this.createAngerBar(2);
         this.createTitleBar();
      ...
      {
         this._tm.addEventListener("timer",this.onTimer);
      ...
      private function onDeposite(param1:MouseEvent) : void
      {
         if(this._isVip == true)
         {
            if((param1.currentTarget as MovieClip).currentFrame <= 2)
            {
               ...
               this._deposite.removeEventListener("click",this.onDeposite);
      ...
      {
         this.setFighter(this._arenaData.leftTeam,1);
         this._leftCapsuleBar.setFighterTeam(this._arenaData.leftTeam);
         this.setFighter(this._arenaData.rightTeam,2);
