package com.taomee.seer2.app.guide.manager
{
   ...
   public class GuideManager
   {
      ...
      private var _guideTip:GuideTip = new GuideTip();
      ...
      private var _btn:TextField = new TextField();
      ...
      {
         super();
         LayerManager.stage.addEventListener("resize",this.updateView);
      }
         ...
         if(this._index < this._guideList.length)
         {
            ...
            if(this.gInfo)
            {
               ...
               if(this.gInfo.targetIndex != -1)
               {
                  ...
                  {
                     this._target.addEventListener("click",this.nextGuide);
                  }
               }
               this.updateView();
            }
      ...
      public function updateView(param1:Event = null) : void
      {
         ...
         _loc4_ = 1200 - LayerManager.stage.stageWidth;
         if(this.gInfo.isLockY == false)
         {
            _loc6_ = 660 - LayerManager.stage.stageHeight;
         }
         if(this.gInfo.isLockX == false)
         {
            if(this.gInfo.isModule == false)
            {
               _loc5_ = new Rectangle(this.gInfo.pointX - _loc4_,this.gInfo.pointY - _loc6_,this.gInfo.targetWidth,this.gInfo.targetHeight);
            }
            else
            {
               if(this.gInfo.moduleWidth >= 1200)
               {
                  _loc3_ = int(LayerManager.stage.stageWidth <= 960 ? 960 : LayerManager.stage.stageWidth);
                  _loc2_ = int(LayerManager.stage.stageHeight <= 560 ? 560 : LayerManager.stage.stageHeight);
               }
               else
               {
                  _loc3_ = LayerManager.stage.stageWidth < this.gInfo.moduleWidth ? int(this.gInfo.moduleWidth) : LayerManager.stage.stageWidth;
                  _loc2_ = LayerManager.stage.stageHeight < this.gInfo.moduleHight ? int(this.gInfo.moduleHight) : LayerManager.stage.stageHeight;
               }
               _loc4_ = 1200 - _loc3_;
               _loc6_ = 660 - _loc2_;
               _loc5_ = new Rectangle(this.gInfo.pointX - _loc4_ / 2,this.gInfo.pointY - _loc6_ / 2,this.gInfo.targetWidth,this.gInfo.targetHeight);
            }
         }
         else
         {
            _loc4_ = this.gInfo.pointX;
            _loc5_ = new Rectangle(this.gInfo.pointX,this.gInfo.pointY - _loc6_,this.gInfo.targetWidth,this.gInfo.targetHeight);
         }
         if(!this._parent)
         {
            return;
         }
         this._mask = AdjectiveMask.instance.makeMask(Vector.<Rectangle>([_loc5_]),0,0,this._parent.width,this._parent.height);
         this._parent.addChild(this._mask);
         if(this.gInfo.dir > 0)
         {
            this._parent.addChild(this._guideTip);
            this._guideTip.showTip(Vector.<Rectangle>([_loc5_]),this.gInfo.dir);
         }
      }
      ...
      public function nextGuide(param1:MouseEvent = null) : void
      {
         if(param1)
         {
            param1.currentTarget.removeEventListener("click",this.nextGuide);
         }
      ...
      public function addGuide2Target(param1:*, param2:int, param3:int, param4:Point, param5:Boolean = false, param6:Boolean = true, param7:int = 8, param8:Boolean = false, param9:Boolean = false, param10:Boolean = false, param11:Number = 0, param12:Number = 0) : void
      {
         ...
         _loc13_ = new GuideInfo();
         _loc13_.parentIndex = param2;
      ...
      private function addFullScreenSkipBtn() : void
      {
         if(this._skipBtn == null)
         {
            ...
            this._skipBtn.addEventListener("click",this.onFullScreenSkillBtnClick);
         }
