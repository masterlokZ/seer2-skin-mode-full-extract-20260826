package com.taomee.seer2.app.leitingevolution
{
   ...
   public class LeiTingEvolutionManager
   {
      ...
      private static var fightCountArr:Array = [];
      ...
      public static function setup() : void
      {
         ...
         DayLimitListManager.getDaylimitList(DAY_LIST,function(param1:DayLimitListInfo):void
         {
            ...
            if(fightCountArr[0] - _loc3_ <= 0)
            {
               ...
               _npc2.addEventListener("click",dialogShow0);
            ...
            if(fightCountArr[1] - _loc3_ <= 0)
            {
               ...
               _npc195.addEventListener("click",dialogShow1);
               MobileManager.addMobile(_npc195,"npc");
            }
            if(fightCountArr[2] - _loc3_ <= 0)
            {
               ...
               _npc6.addEventListener("click",dialogShow2);
               MobileManager.addMobile(_npc6,"npc");
            }
            if(fightCountArr[3] - _loc3_ <= 0)
            {
               ...
               _npc153.addEventListener("click",dialogShow3);
               MobileManager.addMobile(_npc153,"npc");
            }
         });
         if(SceneManager.prevSceneType == 2)
         {
            if(Fight_ID.indexOf(FightManager.currentFightRecord.initData.positionIndex) != -1)
            {
               ...
               DayLimitListManager.getDaylimitList(DAY_LIST,function(param1:DayLimitListInfo):void
               {
                  ...
                  var _loc4_:int = getMinFromArr(fightCountArr);
                  var _loc3_:int = 0;
                  _loc2_ = 0;
                  while(_loc2_ < 4)
                  {
                     if(_loc4_ == fightCountArr[_loc2_])
                     {
                        _loc3_++;
                     }
                     _loc2_++;
      ...
      public static function dispose() : void
      {
         ...
         if(_npc195)
         {
            MobileManager.removeMobile(_npc195,"npc");
            _npc195 = null;
         }
         if(_npc6)
         {
            MobileManager.removeMobile(_npc6,"npc");
            _npc6 = null;
         }
         if(_npc153)
         {
            MobileManager.removeMobile(_npc153,"npc");
            _npc153 = null;
         }
      }
   }
