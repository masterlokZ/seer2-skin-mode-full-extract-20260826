package com.taomee.seer2.app.actor
{
   ...
   public class Actor extends PathMobile
   {
      ...
      protected var _floatList:Array = [100324,100377,100393,100394,100419,100439,100464,100512,100582,100568,100569,100595,100690];
      ...
      public function updateNick(param1:String) : void
      {
         ...
         if(this._userInfo.vipInfo.isVip())
         {
            ...
            {
               this._nickImage = LabelRenderer.getYearVipImage(value,16711680,3342336);
               ServerBufferManager.getServerBuffer(340,function(param1:ServerBuffer):void
               {
                  var server:ServerBuffer = param1;
                  var _isPlay:Boolean = Boolean(server.readDataAtPostion(8));
                  if(!_isPlay)
                  {
                     ServerBufferManager.updateServerBuffer(340,8,1);
            ...
            {
               this._nickImage = LabelRenderer.getLabelImage(value,16776960,3342336);
            }
         }
         else
         {
      ...
      public function walk(param1:Number, param2:Number) : void
      {
         ...
         if(Boolean(SceneManager.active) && SceneManager.active.mapModel.sceneType == 7)
         {
            _loc4_ = new Point(this.x,this.y);
            _loc3_ = new Point(param1,param2);
            SceneManager.active.mapModel.gameMap.walk(this,_loc4_,_loc3_);
         }
         else
         {
      ...
      {
         Connection.send(CommandSet.SYNC_POSITION_1101,param1,param2,getTimer() / 1000);
      }
      ...
      private function onComplete() : void
      {
         ...
         this._bar.scaleX = this._leaveTime / 300;
         this._haveAward.addEventListener("click",this.onHaveAward);
      }
      ...
      private function candyActDispose() : void
      {
         ...
         if(this._haveAward)
         {
            this._haveAward.removeEventListener("click",this.onHaveAward);
      ...
      public function transformAsRabbit(param1:int) : void
      {
         ...
         SwapManager.swapItem(this.RABBIT_SWAP_LIST[1],1,function(param1:IDataInput):void
         {
            ...
            DayLimitListManager.getDaylimitList(RABBIT_DAY_LIST,function(param1:DayLimitListInfo):void
            {
               ...
               _transTime = 300 - info.getCount(RABBIT_DAY_LIST[1]);
      ...
      private function onRabbitIconLoaded() : void
      {
         ...
         this._goPanel.addEventListener("click",this.onGoPanel);
      }
      ...
      private function transformTime(param1:int) : void
      {
         ...
         this._transTimeBar.scaleX = (300 - this._transTime) / 300;
      }
      ...
      public function transformDispose() : void
      {
         if(this._goPanel)
         {
            this._goPanel.removeEventListener("click",this.onGoPanel);
      ...
      private function initEventListener() : void
      {
         ...
         this._hotObject.addEventListener("click",this.onMouseClick);
         this._userInfo.addEventListener("followingPetUpdate",this.onFollowingPetUpdate);
         this._userInfo.addEventListener("RIDING_PET_UPDATE",this.onRidePetUpdate);
         this._userInfo.addEventListener("equipUpdate",this.onEquipUpdate);
         this._userInfo.addEventListener("trainerScoreUpdate",this.onTrainerScoreUpdate);
         this._userInfo.vipInfo.addEventListener("vipUpdate",this.onVipUpdate);
      ...
      private function disposeEventListener() : void
      {
         ...
         this._hotObject.removeEventListener("click",this.onMouseClick);
         this._userInfo.removeEventListener("followingPetUpdate",this.onFollowingPetUpdate);
         this._userInfo.removeEventListener("RIDING_PET_UPDATE",this.onRidePetUpdate);
         this._userInfo.removeEventListener("equipUpdate",this.onEquipUpdate);
         this._userInfo.removeEventListener("trainerScoreUpdate",this.onTrainerScoreUpdate);
         this._userInfo.vipInfo.removeEventListener("vipUpdate",this.onVipUpdate);
         ...
         if(Boolean(SceneManager.active) && SceneManager.active.mapID == 3845)
         {
            if(this.getInfo().id != ActorManager.getActor().id)
            {
               if(this.getInfo().activityData[17] != ActorManager.getActor().getInfo().activityData[17])
               {
                  ...
                  ModelLocator.getInstance().dispatchEvent(new LogicEvent("godFirePkSelect",_loc2_));
               }
               else
               {
         ...
         if(Boolean(SceneManager.active) && SceneManager.active.mapID == 3852)
         {
            if(this.getInfo().id != ActorManager.getActor().id)
            {
               if(this.getInfo().activityData[20] != ActorManager.getActor().getInfo().activityData[20])
               {
                  ...
                  ModelLocator.getInstance().dispatchEvent(new LogicEvent("bestCatchPkSelect",_loc3_));
               }
               else
               {
                  AlertManager.showAlert("这个赛尔是你的队友哦！请挑战其他阵营的成员！");
      ...
      public function startTransform(param1:int, param2:String = "pet") : void
      {
         ...
         if(this._transformMobile.hasEventListener("click") == false)
         {
            this._transformMobile.addEventListener("click",this.onMouseClick);
         }
      ...
      public function stopTransform() : void
      {
         if(this._isTransform)
         {
            ...
            {
               this._transformMobile.removeEventListener("click",this.onMouseClick);
      ...
      private function onRidePetUpdate(param1:UserInfoEvent) : void
      {
         ...
         var _loc5_:Vector.<EquipItem> = this.getInfo().equipVec;
         if(this._userInfo.ridingPetInfo)
         {
            if(ActorManager.actorInfo.vipInfo.isVip() == false)
            {
               if(TimeManager.getServerTime() - this._userInfo.ridingPetInfo.chipPutOnTime > 604800)
               {
                  return;
               }
            }
            _loc7_ = PetRideShopConfig.getEquipIdByChipBackId(this._userInfo.ridingPetInfo.petRideChipId);
            _loc6_ = new EquipItem(_loc7_);
            if(this._userInfo.ridingPetInfo.isPetRiding)
            {
               ...
               this.getInfo().updateEquipVec(_loc5_);
            }
         }
         else
         {
            ...
            {
               _loc2_ = _loc5_[_loc3_].slotIndex == 11 || _loc5_[_loc3_].slotIndex == 3;
               _loc4_ = PetRideShopConfig.isRidePetEquip(_loc5_[_loc3_].referenceId);
               if(_loc2_ && _loc4_)
               {
