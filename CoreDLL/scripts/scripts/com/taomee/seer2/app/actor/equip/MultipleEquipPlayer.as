package com.taomee.seer2.app.actor.equip
{
   ...
   public class MultipleEquipPlayer extends Sprite implements IAnimation
   {
      ...
      public function hideHandAndFootEquipPlayer() : void
      {
         ...
         {
            if(_loc1_.slotIndex == 10 || _loc1_.slotIndex == 2 || _loc1_.slotIndex == 9 || _loc1_.slotIndex == 6)
            {
      ...
      private function arrangeSoltDepth() : void
      {
         ...
         var _loc2_:int = int(this._playerVec.length);
         var _loc1_:int = 0;
         while(_loc1_ < _loc2_)
         {
            _loc3_ = this._playerVec[_loc1_];
            if(getChildAt(_loc1_) != _loc3_)
            {
               addChildAt(_loc3_,_loc1_);
            }
            _loc1_++;
         }
      }
      ...
      {
         var _loc4_:uint = uint(EquipSlotDepth.getDepthByDirection(param1.slotIndex,this._actor.direction));
         var _loc3_:uint = uint(EquipSlotDepth.getDepthByDirection(param2.slotIndex,this._actor.direction));
         if(_loc4_ > _loc3_)
         {
            return 1;
         }
