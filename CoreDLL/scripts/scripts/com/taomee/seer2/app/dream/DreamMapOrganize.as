package com.taomee.seer2.app.dream
{
   ...
   public class DreamMapOrganize extends Sprite
   {
      ...
      public function DreamMapOrganize()
      {
         ...
         this._dreamRecoverBtn.addEventListener("click",this.onRecover);
      ...
      private function hideMapContent() : void
      {
         ...
         var _loc2_:Sprite = SceneManager.active.mapModel.content;
         var _loc1_:int = _loc2_.numChildren;
         var _loc3_:int = 0;
         while(_loc3_ < _loc1_)
         {
      ...
      private function hasGivenName(param1:InteractiveObject) : Boolean
      {
         ...
         if(_loc3_ == null || _loc3_ == "" || StringUtil.beginsWith(_loc3_,_loc2_) || _loc3_ == "离开梦境")
         {
            return false;
         }
         return true;
      ...
      private function dreamEntryMap() : void
      {
         ...
         if(SceneManager.prevSceneType == 2)
         {
            ...
            {
               this.disposeDream();
               SceneManager.changeScene(1,3840);
               return;
            }
      ...
      private function showBox() : void
      {
         if(_mapInfo.goldenBoxCount != 0 || _mapInfo.sliverBoxCount != 0)
         {
            ...
            {
               var _loc2_:int = 0;
               while(_loc2_ < _mapInfo.goldenBoxCount)
               {
                  new DreamBox(0);
                  _loc2_++;
               }
               var _loc1_:int = 0;
               while(_loc1_ < _mapInfo.sliverBoxCount)
               {
                  new DreamBox(1);
                  _loc1_++;
               }
         ...
         if(this._info.id == 1)
         {
            DreamOneEntry.getCurrQuestIndex(function(param1:int):void
            {
               ...
               if(param1 == 4)
               {
                  ...
                  ServerBufferManager.updateServerBuffer(350,4,1);
               }
               else if(param1 == 5)
               {
                  _loc2_ = 0;
                  while(_loc2_ < _mapInfo.list.length)
                  {
                     if(_mapInfo.list[_loc2_].isBoss == 0)
                     {
                        ...
                        if(_loc3_ == 0)
                        {
                           UILoader.load(URLUtil.getRes("dream/dreamOneEntry.swf"),"domain",onLoadAnimation);
                        }
                     }
                     _loc2_++;
                  }
      ...
      private function onLoadAnimation(param1:ContentInfo) : void
      {
         ...
         MovieClipUtil.playMc(playMC,2,playMC.totalFrames,function():void
         {
            ...
            ServerBufferManager.updateServerBuffer(350,5,1);
         },true);
      }
      ...
      {
         var _loc4_:DreamPet = null;
         var _loc3_:DreamPetInfo = null;
         var _loc5_:int = 0;
         var _loc2_:DreamPet = null;
         var _loc1_:int = 0;
         this._bossInfoList = Vector.<DreamPetInfo>([]);
         this._bossList = Vector.<DreamPet>([]);
         var _loc6_:int = 0;
         while(_loc6_ < _mapInfo.list.length)
         {
            _loc5_ = _mapInfo.list[_loc6_].totalPetCount - _mapInfo.list[_loc6_].killPetCount;
            if(_mapInfo.list[_loc6_].isBoss == 0)
            {
               _loc1_ = 0;
               while(_loc1_ < _loc5_)
               {
                  _loc2_ = new DreamPet(_mapInfo.list[_loc6_].petId,_mapInfo.fightId,0,false);
                  MobileManager.addMobile(_loc2_,"spawnedPet");
                  _loc1_++;
               }
            }
            else if(_mapInfo.list[_loc6_].killPetCount < 1)
            {
               _loc3_ = this._info.getBossInfo(_mapInfo.list[_loc6_].petId);
               _loc4_ = new DreamPet(_mapInfo.list[_loc6_].petId,_mapInfo.fightId,_loc3_.level,true,_mapInfo);
               this._bossList.push(_loc4_);
               MobileManager.addMobile(_loc4_,"spawnedPet");
               this._bossInfoList.push(_loc3_);
            }
            _loc6_++;
         }
      }
      ...
      private function onAddAllPetBlood(param1:MessageEvent) : void
      {
         ...
         for each(_loc5_ in _loc3_)
         {
            ...
            PetInfoManager.dispatchEvent("petPropertiesChange",_loc5_);
         }
      ...
      public function onPetListShow() : void
      {
         ...
         while(_loc3_ < _loc2_.length)
         {
            ...
            instance.addChild(_loc4_);
            this._petList.push(_loc4_);
            _loc4_.x = 100;
            _loc4_.y = 2 + (_loc4_.height + 2) * _loc3_ + 37;
