package com.taomee.seer2.app.config
{
   ...
   public class LeiyiKaisaGadEndConfig
   {
      ...
      {
         QueueLoader.load(URLUtil.getActivityXML("LeiyiKaisaGadEnd"),"text",this.onComplete);
      }
      ...
      {
         var _loc2_:XML = null;
         var _loc1_:LeiyiKaisaGadEndInfo = null;
         this._allChip = new Vector.<LeiyiKaisaGadEndInfo>();
         for each(_loc2_ in this._allXml)
         {
            _loc1_ = new LeiyiKaisaGadEndInfo();
            _loc1_.index = int(_loc2_.@index);
            _loc1_.itemID = int(_loc2_.@itemID);
            _loc1_.mineID = int(_loc2_.@mineID);
            _loc1_.x = Number(_loc2_.@y);
            _loc1_.y = Number(_loc2_.@y);
            _loc1_.scale = Number(_loc2_.@scale);
            _loc1_.name = String(_loc2_.@name);
            this._allChip.push(_loc1_);
         }
         this.callback();
      }
   }
