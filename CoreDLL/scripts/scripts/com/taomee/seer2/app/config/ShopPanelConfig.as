package com.taomee.seer2.app.config
{
   ...
   public class ShopPanelConfig
   {
      ...
      private static function entryMi(param1:Vector.<ShopItemInfo>, param2:Boolean = false) : void
      {
         ...
         {
            _loc4_ = new ShopItemInfo();
            _loc4_.referenceId = uint(_loc3_.attribute("referenceId"));
            _loc4_.price = Number(_loc3_.attribute("price"));
            if(!_allShopItemMap.getValue(_loc4_.referenceId))
            {
               _allShopItemMap.add(_loc4_.referenceId,_loc4_);
            }
            if(String(_loc3_.attribute("name")) != "")
            {
               _loc4_.name = String(_loc3_.attribute("name"));
            }
            _loc4_.itemType = "normal";
            if(param2)
            {
               _loc4_.limitedTotalCountTxt = uint(_loc3_.attribute("limitedTotalCountTxt"));
               _loc4_.itemType = "limited";
            }
            if(String(_loc3_.attribute("type")) != "")
            {
               _loc4_.itemType = String(_loc3_.attribute("type"));
               _loc4_.limitedTotalCountTxt = uint(_loc3_.attribute("limitedTotalCountTxt"));
            }
            if(String(_loc3_.attribute("tip")) != "")
            {
               _loc4_.tip = String(_loc3_.attribute("tip"));
            }
            else
            {
               _loc4_.tip = ItemConfig.getItemDefinition(_loc4_.referenceId).tip;
            }
            if(String(_loc3_.attribute("discount")) != "")
            {
               _loc4_.discount = Number(_loc3_.attribute("discount")) / 10;
            }
            else
            {
               _loc4_.discount = 1;
            ...
            {
               _loc4_.isDown = uint(_loc3_.attribute("down"));
            }
            else
            {
               _loc4_.isDown = 0;
            }
            if(String(_loc3_.attribute("isHelp")) != "")
            {
               _loc4_.isHelp = uint(_loc3_.attribute("isHelp"));
            }
            else
            {
               _loc4_.isHelp = 0;
            }
            if(String(_loc3_.attribute("twoGround")) != "")
            {
               _loc4_.isTwoGro = uint(_loc3_.attribute("twoGround"));
            }
            else
            {
               _loc4_.isTwoGro = 0;
            }
            if(String(_loc3_.attribute("noVipDiscount")) != "")
            {
               _loc4_.noVipDiscount = Number(_loc3_.attribute("noVipDiscount")) / 10;
            }
            else
            {
               _loc4_.noVipDiscount = 1;
            }
            _loc4_.shopType = "mi";
            param1.push(_loc4_);
         }
      ...
      public static function getCoinsEquipInfoList(param1:uint) : Vector.<ShopItemInfo>
      {
         ...
         var _loc4_:uint = 0;
         var _loc2_:ShopItemInfo = null;
         var _loc3_:Vector.<ShopItemInfo> = Vector.<ShopItemInfo>([]);
         for each(_loc2_ in _equip)
         {
            switch((_loc5_ = ItemConfig.getItemDefinition(_loc2_.referenceId) as EquipItemDefinition).slotIndex - 1)
            {
               case 0:
                  _loc4_ = 1;
                  break;
      ...
      private static function allShop() : Vector.<ShopItemInfo>
      {
         ...
         for each(_loc1_ in _newShopList)
         {
            _loc2_.add(_loc1_.referenceId,_loc1_);
         }
         for each(_loc4_ in _limitedShopList)
         {
            _loc2_.add(_loc4_.referenceId,_loc4_);
         }
         for each(_loc3_ in _hotShop)
         {
            _loc2_.add(_loc3_.referenceId,_loc3_);
         }
         return Vector.<ShopItemInfo>(_loc2_.getValues());
      }
