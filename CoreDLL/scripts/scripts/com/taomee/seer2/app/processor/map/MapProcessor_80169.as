package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80169 extends MapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         this.mobile = MobileManager.getMobile(734,"npc");
         if(this.mobile)
         {
            this.mobile.buttonMode = true;
            this.mobile.addEventListener("click",this.onClick);
         }
         this.mobile1 = MobileManager.getMobile(735,"npc");
         if(this.mobile1)
         {
      ...
      private function checkIsInTime() : Boolean
      {
         ...
         _loc3_ = _loc4_ == 2014 && _loc7_ == 8 && _loc6_ == 30;
         if(_loc3_)
         {
            return true;
         }
         ...
         NpcDialog.show(734,"太阳神阿波罗",[[0,"生命光轮不止，太阳神的荣耀与你同在！在战斗中燃烧光耀吧！（VIP全员战斗伤害加成+5%，年费VIP+10%，拥有太阳神之子米瑞尔进入弱化挑战）"]],["开始挑战","准备一下","加入VIP"],[function():void
         {
            ActiveCountManager.requestActiveCountList([203944,203945],function(param1:Parser_1142):void
            {
               ...
               if(leftNum > 0)
               {
                  if(checkIsInTime())
                  {
                     FightManager.startFightWithWild(867);
                  }
      ...
      override public function dispose() : void
      {
         if(this.mobile)
         {
            this.mobile.removeEventListener("click",this.onClick);
         }
         super.dispose();
      }
   }
