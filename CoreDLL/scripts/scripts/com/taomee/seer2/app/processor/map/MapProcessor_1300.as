package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_1300 extends MapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         var _loc2_:MovieClip = _map.content["eft"] as MovieClip;
         _loc2_.mouseEnabled = _loc2_.mouseChildren = false;
         var _loc1_:int = Math.random() * 10;
      ...
      {
         if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.type == "fightPVP" && FightManager.getPositionIndex() == 5)
         {
      ...
      {
         this.rank.addEventListener("click",this.showRank);
      ...
      protected function onDiaLogShow(param1:DialogPanelEvent) : void
      {
         ...
         {
            _loc2_ = new CustomUnit("module","我要竞技","我要竞技");
      ...
      private function onCustomUnitClick(param1:DialogPanelEvent) : void
      {
         ...
         if(str == "我要竞技")
         {
            ServerBufferManager.getServerBuffer(385,function(param1:ServerBuffer):void
            {
               var server:ServerBuffer = param1;
               if(server.readDataAtPostion(1) == 0)
               {
                  ModuleManager.addEventListener("NewSkyTipPanel","hide",(function():*
                  {
                     var onTipHide:Function;
                     return onTipHide = function(param1:ModuleEvent):void
                     {
                        ModuleManager.showAppModule("SkySportsPanel");
                     };
                  })());
                  ModuleManager.showAppModule("NewSkyTipPanel");
                  ServerBufferManager.updateServerBuffer(385,1,1);
               }
               else
               {
      ...
      {
         super.dispose();
         this.rank.removeEventListener("click",this.showRank);
