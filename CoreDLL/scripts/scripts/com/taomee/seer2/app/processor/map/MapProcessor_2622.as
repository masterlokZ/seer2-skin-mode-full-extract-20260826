package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_2622 extends MapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         _loc2_.setTime(TimeManager.getPrecisionServerTime() * 1000);
         var _loc1_:Boolean = _loc2_.hours >= 13 && _loc2_.hours < 14;
         var _loc3_:Boolean = _loc2_.hours >= 18 && _loc2_.hours < 19;
         if(!_loc1_ && !_loc3_)
         {
            this.alertTime();
         }
         ...
         this.leftTime = (60 - _loc2_.minutes) * 60 + (60 - _loc2_.seconds);
      ...
      private function updateIcon(param1:MessageEvent) : void
      {
         ...
         {
            if(_loc3_.getInfo().activityData[10] == 1)
            {
               _loc3_.addEggIcon("UI_WorldWarA");
            }
            else if(_loc3_.getInfo().activityData[10] == 2)
            {
               _loc3_.addEggIcon("UI_WorldWarB");
            }
         }
         if(ActorManager.actorInfo.activityData[10] == 1)
         {
            ActorManager.getActor().addEggIcon("UI_WorldWarA");
         }
         else if(ActorManager.actorInfo.activityData[10] == 2)
         {
      ...
      private function shootComplete(param1:ShootEvent) : void
      {
         ...
         if(_loc2_.userID == ActorManager.actorInfo.id)
         {
            ...
            {
               if(this.actorList[_loc3_].hitTestPoint(_loc2_.endPos.x,_loc2_.endPos.y) && this.actorList[_loc3_].getInfo().activityData[10] != ActorManager.actorInfo.activityData[10])
               {
                  SwapManager.swapItem(2720,1,this.swapComplete,null,new SpecialInfo(1,this.actorList[_loc3_].getInfo().id));
                  break;
               }
               _loc3_++;
            }
         ...
         else
         {
            ...
            if(ActorManager.getActor().hitTestPoint(_loc2_.endPos.x,_loc2_.endPos.y) && this.actorList[_loc3_].getInfo().activityData[10] != ActorManager.actorInfo.activityData[10])
            {
               ++this.shootedNum;
               this.checkShooted();
            }
      ...
      private function checkShooted() : void
      {
         ...
         {
            SceneManager.addEventListener("switchComplete",this.showPanel);
            SceneManager.changeScene(8,ActorManager.actorInfo.id);
         }
      }
