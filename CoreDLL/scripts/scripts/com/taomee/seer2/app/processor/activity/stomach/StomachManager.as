package com.taomee.seer2.app.processor.activity.stomach
{
   ...
   public class StomachManager
   {
      ...
      public static function isTimerLater() : Boolean
      {
         ...
         {
            if(DateUtil.inInDateScope(5,13,0,15,1) || DateUtil.inInDateScope(6,13,0,15,1) || DateUtil.inInDateScope(0,13,0,15,1))
            {
               return false;
            }
         }
      ...
      {
         this._map.content.addEventListener("enterFrame",this.onEnter);
      }
      ...
      {
         this._grassNpc = MobileManager.getMobile(62,"npc");
         this._fireNpc = MobileManager.getMobile(63,"npc");
         this._waterNpc = MobileManager.getMobile(64,"npc");
         this._fahaiNpc = MobileManager.getMobile(61,"npc");
         this._grassNpc.buttonMode = true;
         this._fireNpc.buttonMode = true;
         this._waterNpc.buttonMode = true;
         this._grassNpc.addEventListener("click",this.onGrass,false,1);
         this._fireNpc.addEventListener("click",this.onFire,false,1);
         this._waterNpc.addEventListener("click",this.onWater,false,1);
      }
      
      private function initDialogEvent() : void
      {
         NpcManager.addEventListener("actorArrive",this.onDialogShow);
      }
      ...
      private function onGrass(param1:MouseEvent) : void
      {
         ...
         if(PetInfoManager.getFirstPetInfo().getPetDefinition().type == 2)
         {
      ...
      private function onFire(param1:MouseEvent) : void
      {
         var event:MouseEvent = param1;
         event.stopImmediatePropagation();
         if(PetInfoManager.getFirstPetInfo().getPetDefinition().type == 3)
         {
      ...
      private function onWater(param1:MouseEvent) : void
      {
         var event:MouseEvent = param1;
         event.stopImmediatePropagation();
         if(PetInfoManager.getFirstPetInfo().getPetDefinition().type == 4)
         {
      ...
      {
         NpcManager.removeEventListener("actorArrive",this.onDialogShow);
         this._map.content.removeEventListener("enterFrame",this.onEnter);
         this._grassNpc.removeEventListener("click",this.onGrass);
         this._fireNpc.removeEventListener("click",this.onFire);
         this._waterNpc.removeEventListener("click",this.onWater);
      }
   }
}
