package com.taomee.seer2.app.processor.activity.fightHolySkyAct
{
   ...
   public class FightHolySkyAct
   {
      ...
      private function initEvent() : void
      {
         ...
         {
            _loc1_.addEventListener("click",this.onDoor);
         }
         this._awardBag.addEventListener("click",this.onAwardBag);
      }
      ...
      private function stateHandle() : void
      {
         if(this._par.infoVec[3] <= 3)
         {
            ...
            MovieClipUtil.playMc(this._eventPetList[this._par.infoVec[3] - 1],2,this._eventPetList[this._par.infoVec[3] - 1].totalFrames,function():void
            {
               ...
               _eventPetList[_par.infoVec[3] - 1].addEventListener("click",onEventPetClick);
            });
         }
         else
         {
            this.setDoorEnable(true);
            FightManager.startFightWithWild(1605);
         }
      }
         ...
         NpcDialog.show(902,"汤姆",[[1,"吖，给我条小黄鱼，我也会送你个礼物"]],["给你！","我要想想"],[function():void
         {
            ItemManager.requestItemList(function():void
            {
               ...
               else
               {
                  ...
                  {
                     SceneManager.changeScene(1,152);
                  },function():void
                  {
                     ShopManager.buyItemForId(MI_ID_LIST[0],function():void
                     {
                        _eventPetList[_index].removeEventListener("click",onEventPetClick);
                        _eventPetList[_index].visible = false;
                        goalItemNumHandle();
                        update();
                     });
      ...
      public function dispose() : void
      {
         ...
         if(this._doorList)
         {
            for each(_loc2_ in this._doorList)
            {
               _loc2_.removeEventListener("click",this.onDoor);
            }
         }
         if(this._awardBag)
         {
            this._awardBag.removeEventListener("click",this.onAwardBag);
            this._awardBag = null;
         }
         if(this._eventPetList)
         {
            for each(_loc1_ in this._eventPetList)
            {
               _loc1_.removeEventListener("click",this.onEventPetClick);
            }
         }
      }
   }
