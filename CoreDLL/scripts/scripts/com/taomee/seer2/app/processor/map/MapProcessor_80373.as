package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80373 extends MapProcessor
   {
      ...
      {
         super.dispose();
         if(_map.hasEventListener("enterFrame"))
         {
            _map.removeEventListener("enterFrame",this.onFrame2);
         }
      }
      ...
      private function update() : void
      {
         ...
         {
            var _loc5_:int = 0;
            var _loc3_:int = int(param1.infoVec[0]);
            ...
            while(_loc5_ < 4)
            {
               _loc2_.push(_loc3_ & Math.pow(2,_loc5_));
               _loc5_++;
            }
            gameStop = 0;
            _loc5_ = 0;
      ...
      private function showlj() : void
      {
         ...
         _map.content["lj1"].addEventListener("click",this.onClick);
         _map.content["lj2"].addEventListener("click",this.onClick);
         _map.content["lj3"].addEventListener("click",this.onClick);
         if(!_map.hasEventListener("enterFrame"))
         {
            _map.addEventListener("enterFrame",this.onFrame2);
         }
      }
      ...
      private function showMovieClip() : void
      {
         ...
         MovieClipUtil.playToEnd(_map.content["movieClip1"],1,function():void
         {
            if(timet == 0)
            {
               ...
               {
                  new SwapInfo(param1);
                  SceneManager.changeScene(1,3833);
      ...
      private function createNpc() : void
      {
         if(!this._npc)
         {
            ...
            this._npc.addEventListener("click",this.initStep2Continue);
         }
      }
      ...
      private function onFrame2(param1:Event) : void
      {
         ...
         if(_map.content["lj1"].currentFrame == _map.content["lj1"].totalFrames && _map.content["lj2"].currentFrame == _map.content["lj2"].totalFrames && _map.content["lj3"].currentFrame == _map.content["lj3"].totalFrames)
         {
            ...
            _map.content["lj2"].visible = false;
            _map.content["lj3"].visible = false;
            _map.content["lj1"].removeEventListener("click",this.onClick);
            _map.content["lj2"].removeEventListener("click",this.onClick);
            _map.content["lj3"].removeEventListener("click",this.onClick);
         }
      }
      ...
      {
         SceneManager.addEventListener("switchComplete",this.fightOver);
      ...
      private function fightOver(param1:SceneEvent) : void
      {
         if(SceneManager.prevSceneType == 2)
         {
            SceneManager.removeEventListener("switchComplete",this.fightOver);
