package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80221 extends MapProcessor
   {
      ...
      {
         if(!_map.front["pet"].hasEventListener("click"))
         {
            _map.front["pet"].addEventListener("click",this.onClickPet);
         }
         if(!_map.front["tarMc"]["leaveBtn"].hasEventListener("click"))
         {
            _map.front["tarMc"]["leaveBtn"].addEventListener("click",this.onClick);
         }
         if(!_map.front["vipBtn"].hasEventListener("click"))
         {
            _map.front["vipBtn"].addEventListener("click",this.onClick2);
         }
      }
      ...
      private function onSpeak1() : void
      {
         ...
         SceneManager.addEventListener("switchComplete",this.onOver);
         this.isToFight = true;
         havewin = false;
         FightManager.startFightWithWild(1094);
      }
      
      private function onOver(param1:Event) : void
      {
         if(FightManager.getPositionIndex() != 1094)
         {
            return;
         }
      ...
      private function update() : void
      {
         ...
         DayLimitManager.getDoCount(1231,function(param1:int):void
         {
            var val:int = param1;
