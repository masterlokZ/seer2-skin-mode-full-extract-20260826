package com.taomee.seer2.app.processor.quest.handler
{
   ...
   public class BranchQuestMapHandler extends QuestMapHandler
   {
      ...
      {
         this._npc.addEventListener("click",this.onQuestClick);
         QuestManager.addEventListener("accept",this.onQuestAccept);
         QuestManager.addEventListener("stepComplete",this.onSepComplete);
      ...
      protected function clearQuest() : void
      {
         ...
         QuestManager.removeEventListener("stepComplete",this.onSepComplete);
         QuestManager.removeEventListener("complete",this.onQuestComplete);
         if(this._npc)
         {
            this._npc.removeEventListener("click",this.onQuestClick);
            this._npc.dispose();
         }
         this._npc = null;
