package com.taomee.seer2.app.processor.quest.handler.branch.quest10024
{
   ...
   public class QuestMapHandler_10024_141 extends QuestMapHandler
   {
      ...
      override public function processMapComplete() : void
      {
         ...
         if(SceneManager.prevSceneType == 2 && QuestManager.getQuest(_quest.id).status == 1)
         {
            this.initNpc();
      ...
      private function initNpc2() : void
      {
         ...
         this._npc2.addEventListener("click",this.onClick2);
      }
      ...
      private function alert() : void
      {
         ...
         {
            _loc1_ = QuestManager.getQuest(_quest.id);
            _loc3_ = _loc1_.getStepData(1,0);
            if(++_loc3_ >= 5)
            {
               _loc1_.setStepData(1,0,_loc3_);
               QuestManager.addEventListener("stepUpdateBuffer",this.onUpdateStepBufferStep1);
               QuestManager.setStepBufferServer(_quest.id,1);
            }
            else
            {
               _loc1_.setStepData(1,0,_loc3_);
               QuestManager.addEventListener("stepUpdateBuffer",this.onUpdateStepBufferStep2);
               QuestManager.setStepBufferServer(_quest.id,1);
            }
         }
      }
      ...
      {
         QuestManager.removeEventListener("stepUpdateBuffer",this.onUpdateStepBufferStep1);
      ...
      {
         QuestManager.removeEventListener("stepUpdateBuffer",this.onUpdateStepBufferStep2);
