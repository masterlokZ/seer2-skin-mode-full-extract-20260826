package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_171 extends MapProcessor
   {
      ...
      private function processBubble() : void
      {
         ...
         this._bubbles.addEventListener("frameConstructed",this.onBubblesFrameConstructed);
         this._bubbles.play();
         this._bubbleTimer = new Timer(60000);
         this._bubbleTimer.addEventListener("timer",this.onBubble);
      ...
      private function onBubblesFrameConstructed(param1:Event) : void
      {
         if(this._bubbleVec == null)
         {
            ...
            this.initBubbleVec();
            this._bubbles.removeEventListener("frameConstructed",this.onBubblesFrameConstructed);
         }
      }
      ...
      private function initBubbleVec() : void
      {
         ...
         while(_loc2_ <= 5)
         {
            _loc1_ = "bubble" + _loc2_;
            _loc3_ = this._bubbles[_loc1_];
            _loc3_.buttonMode = true;
            _loc3_.addEventListener("click",this.onBubbleClick);
      ...
      private function parseRewardStatus() : void
      {
         ...
         if(this._brokenBubbleNum == 3)
         {
            this._brokenBubbleNum = 0;
            if(SwapManager.isSwapNumberMax(42))
            {
               SwapManager.entrySwap(42);
            }
         }
      }
      ...
      {
         this._brokenAnimation.addEventListener("enterFrame",this.onBrokenAnimationEnd);
      ...
      private function onBrokenAnimationEnd(param1:Event) : void
      {
         ...
         {
            this._brokenAnimation.removeEventListener("enterFrame",this.onBrokenAnimationEnd);
      ...
      private function clearBubbles() : void
      {
         ...
         if(this._bubbleVec)
         {
            for each(_loc1_ in this._bubbleVec)
            {
               _loc1_.removeEventListener("click",this.onBubbleClick);
            }
         }
      ...
      override public function dispose() : void
      {
         ...
         this._bubbleTimer.removeEventListener("timer",this.onBubble);
