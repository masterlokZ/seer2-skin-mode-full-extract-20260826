package com.taomee.seer2.app.processor.activity.dreamGift
{
   ...
   public class DreamGiftAct
   {
      ...
      private function clickItemShow() : void
      {
         ...
         var _loc1_:int = 0;
         var _loc3_:int = 0;
         _loc2_ = NUM_LIST[0] - this._info.getCount(DAY_LIST[0]);
         if(_clickIndexList.length == 0)
         {
            _loc1_ = 0;
            while(_loc1_ < NUM_LIST[0])
            {
               if(_isClickItem)
               {
                  if(_loc1_ < _loc2_)
                  {
                     this._clickItemList[_loc1_].visible = true;
                  }
                  else
                  {
         ...
         else
         {
            ...
            while(_loc1_ < NUM_LIST[0])
            {
               if(_clickIndexList.indexOf(_loc1_) != -1)
               {
                  this._clickItemList[_loc1_].visible = false;
               }
      ...
      private function initEvent() : void
      {
         ...
         this._getGift.addEventListener("click",this.onGetGift);
         this._boss.addEventListener("click",this.onBoss);
      ...
      private function onClickItem(param1:MouseEvent) : void
      {
         ...
         if(randomVal <= 80)
         {
            ...
            SwapManager.swapItem(4001,1,function(param1:IDataInput):void
            {
               new SwapInfo(param1);
               _clickIndexList.push(_selectIndex);
               update();
      ...
      private function onGetGift(param1:MouseEvent) : void
      {
         ...
         _isClickItem = true;
         this._clickTip.visible = true;
         this._getGift.removeEventListener("click",this.onGetGift);
      ...
      private function createPet() : void
      {
         ...
         while(_loc2_ < 3)
         {
            _loc1_ = new SpawnedPet(688,FIGHT_INDEX_LIST[2],20);
            _loc1_.name = "草莓猫";
            MobileManager.addMobile(_loc1_,"spawnedPet");
            this._petList.push(_loc1_);
            _loc2_++;
         }
      }
