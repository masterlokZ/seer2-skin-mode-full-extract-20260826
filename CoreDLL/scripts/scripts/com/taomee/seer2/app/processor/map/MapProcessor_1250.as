package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_1250 extends MapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         _loc1_.writeUnsignedInt(762);
         _loc1_.writeUnsignedInt(763);
      ...
      private function checkMobile() : void
      {
         ...
         var _loc5_:int = 0;
         while(_loc5_ < this.currentComplete)
         {
            this.npcList.push(null);
            _loc5_++;
         }
         var _loc7_:int = this.currentComplete;
         while(_loc7_ < 4)
         {
            _loc4_ = new Mobile();
            _loc4_.resourceUrl = URLUtil.getNpcSwf(NPC_ID_LIST[_loc7_]);
            _loc4_.buttonMode = true;
            _loc4_.addEventListener("click",this.toFight);
            _loc4_.setPostion(NPC_POS_LIST[_loc7_]);
            MobileManager.addMobile(_loc4_,"npc");
            this.npcList.push(_loc4_);
            _loc7_++;
         }
         ...
         var _loc6_:Boolean = SceneManager.prevSceneType == 2 ? true : false;
         var _loc2_:Boolean = FightManager.fightWinnerSide == 1 ? true : false;
         var _loc1_:int = int(FightManager.currentFightRecord.initData.positionIndex);
         this._lastFightIndex = FIGHT_ID_LIST.indexOf(_loc1_);
         var _loc3_:Boolean = this.currentComplete == 4 ? true : false;
         this.canFirstSeeBoss = _loc1_ == FIGHT_ID_LIST[3] && _loc3_ && this.bossComplete == 0 ? true : false;
         if(_loc6_ && _loc2_ && this._lastFightIndex >= 0 && this._lastFightIndex <= 3)
         {
            this.showSpriteLight();
         }
         if(_loc3_ && !this.canFirstSeeBoss)
         {
            this.showBoss();
         }
      }
      ...
      private function showSpriteLight() : void
      {
         if(!this._resLib)
         {
            QueueLoader.load(URLUtil.getQuestAnimation("sceneAnimation/1250"),"domain",this.onLoadComplete);
         }
         else
         {
            this.showLight();
      ...
      private function toFight(param1:MouseEvent) : void
      {
         ...
         this.actor.addEventListener("finished",this.onMoveFinish);
      ...
      private function onMoveFinish(param1:MoveEvent) : void
      {
         ...
         this.actor.removeEventListener("finished",this.onMoveFinish);
         x = this.actor.x;
         y = this.actor.y;
      ...
      override public function dispose() : void
      {
         ...
         while(_loc1_ < 5)
         {
            ...
            {
               this.npcList[_loc1_].removeEventListener("click",this.toFight);
