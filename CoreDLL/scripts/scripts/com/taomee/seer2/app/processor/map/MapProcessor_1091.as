package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_1091 extends MapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         this.openPanelBtn.addEventListener("click",this.onClick);
         StatisticsManager.sendNovice("0x1003404E");
      }
      ...
      private function getBackData() : void
      {
         ...
         else
         {
            ...
            if(this.clickTarget)
            {
               this.clickTarget.addEventListener("click",this.onPetClick);
            }
         }
      }
      ...
      {
         QueueLoader.load(URLUtil.getActivityAnimation("PetCardsExchange"),"domain",this.onLoadComplete);
      }
      ...
      private function onLoadComplete(param1:ContentInfo) : void
      {
         ...
         while(_loc3_ < 5)
         {
            ...
            _loc2_.addEventListener("click",this.onPetClick);
            _loc3_++;
         }
      }
      ...
      private function onPetClick(param1:MouseEvent) : void
      {
         ...
         this.clickTarget.removeEventListener("click",this.onPetClick);
         this.getTimeCount();
      }
      ...
      private function onGetRewardSuccess(param1:MessageEvent) : void
      {
         ...
         var _loc5_:Parser_1060 = new Parser_1060(param1.message.getRawData());
         var _loc4_:Vector.<ItemSwapInfo> = _loc5_.addItemVec;
         var _loc2_:int = 0;
         while(_loc2_ < _loc4_.length)
         {
            if(special_item_id_list.indexOf(_loc4_[_loc2_].id) != -1)
            {
               ServerMessager.addMessage("恭喜你获得1个[" + special_item_name_list[special_item_id_list.indexOf(_loc4_[_loc2_].id)] + "字]");
            }
            _loc2_++;
         }
         if(_loc4_ && _loc4_.length >= 1 && (_loc4_[0].id == 1 || _loc4_[0].id == 200016 || _loc4_[0].id == 200201))
         {
            _loc5_.showResult(true,null,true);
         }
         else
         {
            _loc5_.showResult(true,null,false);
         }
         if(this.clickTarget)
         {
            this.clickTarget.addEventListener("click",this.onPetClick);
      ...
      {
         this.openPanelBtn.removeEventListener("click",this.onClick);
         Connection.removeErrorHandler(CommandSet.DIGGER_MINE_1060,this.onGetRewardError);
         Connection.removeCommandListener(CommandSet.DIGGER_MINE_1060,this.onGetRewardSuccess);
         var _loc1_:int = 0;
         while(_loc1_ < this.petList.length)
         {
            this.petList[_loc1_].removeEventListener("click",this.onClick);
