package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_50 extends TitleMapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         StatisticsManager.sendNoviceAccountHttpd("0x1003345C");
      ...
      private function initShowNpc() : void
      {
         ...
         while(_loc1_ < 6)
         {
            _loc2_ = _map.content["btn" + _loc1_];
            TooltipManager.addCommonTip(_loc2_,"神勇骑士铠甲秀");
            _loc2_.addEventListener("click",this.onOpenShowBuyPanel);
            this._npcShow.push(_loc2_);
            _loc1_++;
         }
         this.randomShow();
      }
      ...
      private function timeManage() : void
      {
         ...
         my_Timer.start();
         my_Timer.addEventListener("timer",_showTxt);
         this.randomShow();
      }
      ...
      {
         var _loc2_:SimpleButton = null;
         var _loc1_:int = MathUtil.randomRegion(0,6);
         var _loc3_:int = 0;
         while(_loc3_ < 6)
         {
            _loc2_ = this._npcShow[_loc3_];
            if(_loc3_ == _loc1_)
            {
               _loc2_.visible = true;
            }
      ...
      private function initNpc() : void
      {
         ...
         this.dandan.addEventListener("click",this.clickDandan);
      }
      ...
      private function initChangeEquipment() : void
      {
         ...
         this._leftChangeEquipment.addEventListener("click",this.onChangeEquipmentClick);
         this._rightChangeEquipment.addEventListener("click",this.onChangeEquipmentClick);
      }
      ...
      private function onChangeEquipmentClick(param1:MouseEvent) : void
      {
         ...
         _loc2_.removeEventListener("click",this.onChangeEquipmentClick);
         _loc2_.nextFrame();
      }
      ...
      private function initMakeEquipment() : void
      {
         ...
         this._makeEquipment.addEventListener("click",this.onMakeEquipmentClick);
      }
      ...
      private function onMakeEquipmentClick(param1:MouseEvent) : void
      {
         ...
         {
            this._makeEquipment.removeEventListener("click",this.onMakeEquipmentClick);
         ...
         if(DateUtil.inInDateScope(5,14,11,14,21))
         {
            ServerBufferManager.getServerBuffer(29,function(param1:ServerBuffer):void
            {
               ...
               if(_loc2_ == 0)
               {
                  ...
                  ServerBufferManager.updateServerBuffer(29,15,1);
               }
            });
         }
      }
      ...
      private function initTwoYearNpcHandle(param1:LogicEvent = null) : void
      {
         ...
         this._oldNpc = MobileManager.getMobile(4,"npc");
         if(this._oldNpc)
         {
         ...
         DayLimitListManager.getDaylimitList(this.DAY_LIST,function(param1:DayLimitListInfo):void
         {
            ...
            {
               if(!MobileManager.getMobile(4,"npc"))
               {
