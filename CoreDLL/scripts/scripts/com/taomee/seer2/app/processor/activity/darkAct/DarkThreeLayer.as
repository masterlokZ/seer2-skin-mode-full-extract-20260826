package com.taomee.seer2.app.processor.activity.darkAct
{
   ...
   public class DarkThreeLayer
   {
      ...
      {
         QueueLoader.load(URLUtil.getActivityAnimation("darkAct/darkAct1"),"swf",function(param1:ContentInfo):void
         {
      ...
      private function stateHandle() : void
      {
         ...
         if(_loc1_ == 0)
         {
            ServerBufferManager.getServerBuffer(78,this.onGetServer,false);
         }
      ...
      private function onGetServer(param1:ServerBuffer) : void
      {
         ...
         else
         {
            MovieClipUtil.playFullScreen(URLUtil.getActivityFullScreen("darkThreeFull"),function():void
            {
               ...
               ServerBufferManager.updateServerBuffer(78,1,1);
               addNpc(RES_ID[0],POS);
      ...
      private function addUinit() : void
      {
         ...
         var _loc3_:Vector.<String> = Vector.<String>(["兑换部件","购买暗月石","第三层规则","挑战死神"]);
      ...
      public function removeUnit() : void
      {
         ...
         var _loc2_:Vector.<String> = Vector.<String>(["兑换部件","购买暗月石","第三层规则","挑战死神"]);
      ...
      private function onUint(param1:DialogPanelEvent) : void
      {
         ...
         if(_loc2_ == "readyGo1")
         {
            FightManager.startFightWithWild(363);
         }
      }
      ...
      public function equipUpdate() : void
      {
         ...
         ItemManager.requestItemList(function():void
         {
            var _loc2_:int = 0;
            var _loc1_:int = 0;
            for each(_loc1_ in EQUIP)
            {
               if(ItemManager.getItemByReferenceId(_loc1_))
               {
                  if(_mapModel)
                  {
                     (_mapModel.content["equip"]["equip" + _loc2_] as MovieClip).gotoAndStop(2);
                  }
               }
               _loc2_++;
            }
