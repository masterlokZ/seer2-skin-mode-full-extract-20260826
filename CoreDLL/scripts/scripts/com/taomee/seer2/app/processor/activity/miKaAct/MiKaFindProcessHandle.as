package com.taomee.seer2.app.processor.activity.miKaAct
{
   ...
   public class MiKaFindProcessHandle
   {
      ...
      private function getURL(param1:Function = null) : void
      {
         ...
         QueueLoader.load(URLUtil.getActivityAnimation("miKaFindAct/miKaFindActRes1"),"swf",function(param1:ContentInfo):void
         {
      ...
      private function update() : void
      {
         DayLimitListManager.getDaylimitList(DAY_LIST,function(param1:DayLimitListInfo):void
         {
            ...
            if(_questIndex > 0)
            {
               flag = BitUtil.getBit(info.getCount(DAY_LIST[0]),_questIndex - 1);
               flag2 = BitUtil.getBit(info.getCount(DAY_LIST[0]),_questIndex);
               if(flag && !flag2)
               {
                  initQuestUI();
               }
               if(SceneManager.prevSceneType == 2 && FightManager.fightWinnerSide == 1 && FightManager.currentFightRecord.initData.positionIndex == 236)
               {
                  if(!flag && !flag2)
                  {
            ...
            {
               flag = BitUtil.getBit(info.getCount(DAY_LIST[0]),_questIndex);
               if(!flag)
               {
                  initQuestUI();
               }
            ...
            if(BitUtil.getBit(info.getCount(DAY_LIST[0]),_questIndex))
            {
               ...
               _candleMc.removeEventListener("click",startFireup);
      ...
      private function initQuestUI() : void
      {
         ...
         this._candleMc.addEventListener("click",this.startFireup);
      ...
      {
         this._candleMc.removeEventListener("click",this.startFireup);
      ...
      private function playFullMovie() : void
      {
         MovieClipUtil.playFullScreen(URLUtil.getActivityFullScreen("miKaFindAct/MiKaFindAct1"),function():void
         {
            ...
            if(Boolean(_map) && Boolean(_map.content["mikaNpc"]))
            {
               ...
               (_map.content["mikaNpc"] as MovieClip).addEventListener("click",mikaFight);
            }
         });
      }
      ...
      public function dispose() : void
      {
         ...
         {
            (this._map.content["mikaNpc"] as MovieClip).removeEventListener("click",this.mikaFight);
         }
      }
   }
}
