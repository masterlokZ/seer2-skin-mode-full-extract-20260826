package com.taomee.seer2.app.processor.activity.springFireworks
{
   ...
   public class SpringFireworksManager
   {
      ...
      private static function updateFireworks() : void
      {
         ...
         while(_loc1_ < _count)
         {
            _loc2_ = UIManager.getMovieClip("UI_Firework");
            _loc2_.buttonMode = true;
            _loc2_.x = Math.random() * 800 + 200;
            _loc2_.y = Math.random() * 440 + 100;
            SceneManager.active.mapModel.content.addChild(_loc2_);
            _loc2_.addEventListener("click",onHongbao);
            _hongbaoVec.push(_loc2_);
            _loc1_++;
         }
      }
