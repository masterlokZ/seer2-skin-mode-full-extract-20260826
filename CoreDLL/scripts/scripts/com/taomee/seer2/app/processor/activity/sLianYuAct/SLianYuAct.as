package com.taomee.seer2.app.processor.activity.sLianYuAct
{
   ...
   public class SLianYuAct
   {
      ...
      private static function onComplete(param1:SceneEvent = null) : void
      {
         ...
         update(function():void
         {
            if(_curIndex == 1)
            {
               ...
               {
                  addNpc();
                  _npc.addEventListener("click",onNpc1);
               }
               else
               {
                  addNpc();
      ...
      private static function taskStateUiInit() : void
      {
         ...
         _clickNum = 0;
         _overBtn.addEventListener("click",onOver);
         ModelLocator.getInstance().addEventListener("sLianYuClick",onClickAdd);
      }
      ...
      private static function taskStepHandle() : void
      {
         ...
         if(_info.getCount(DAY_LIST[0]) == 2)
         {
            ...
            _sNextTip.addEventListener("click",onGoFinish);
         }
      }
      ...
      private static function getURL(param1:Function = null) : void
      {
         ...
         QueueLoader.load(URLUtil.getActivityAnimation("SLianYuAct"),"swf",function(param1:ContentInfo):void
         {
      ...
      private static function onNpc(param1:MouseEvent) : void
      {
         ...
         NpcDialog.show(14,"S",[[2,"炼狱大门被雾霾笼罩，我的思念体因为受到神秘黑影的惊吓，身陷危难，小赛尔你愿意去帮助它吗？时间不多，你要尽快回来！"]],["我这就去","那什么……"],[function():void
         {
            SwapManager.swapItem(SWAP_LIST[0],1,function(param1:IDataInput):void
            {
               new SwapInfo(param1);
               removeNpc();
               SceneManager.changeScene(9,MAP_LIST[0]);
            });
         }]);
      }
      ...
      private static function dispose() : void
      {
         ...
         if(_overBtn)
         {
            _overBtn.removeEventListener("click",onOver);
            _overBtn = null;
         }
         ModelLocator.getInstance().removeEventListener("sLianYuClick",onClickAdd);
         ModelLocator.getInstance().removeEventListener("sLianYuUpdate",onUpdate);
         _timeTxt = null;
         _finishNum = null;
         _completeList = null;
         _sNextTip = null;
