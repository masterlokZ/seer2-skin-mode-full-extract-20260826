package com.taomee.seer2.app.processor.activity.party
{
   ...
   public class PartyManager
   {
      ...
      {
         this._morph = MobileManager.getMobile(189,"npc");
         this._npcList = Vector.<Mobile>([]);
         var _loc1_:int = 0;
         while(_loc1_ < 3)
         {
            this._npcList.push(MobileManager.getMobile(537 + _loc1_,"npc"));
            _loc1_++;
         }
      }
      ...
      private function addNpcEvent() : void
      {
         ...
         this._morph.addEventListener("click",this.onMorph);
         for each(_loc1_ in this._npcList)
         {
            _loc1_.addEventListener("click",this.onNpc,false,1);
         }
         DialogPanel.addEventListener("customReplyClick",this.showNpcChallengePanel);
      }
      ...
      {
         if(DateUtil.inInDateScope(6,14,0,16) || DateUtil.inInDateScope(0,14,0,16))
         {
         ...
         if(this._one)
         {
            ...
            SwapManager.swapItem(926,1,function(param1:IDataInput):void
            {
               ...
               MovieClipUtil.playMc(_animation,2,_animation.totalFrames,function():void
               {
                  ...
                  _one = false;
                  ServerBufferManager.updateServerBuffer(64,0,1);
               },true);
            });
         }
         else
      ...
      {
         ServerBufferManager.getServerBuffer(64,function(param1:ServerBuffer):void
         {
      ...
      private function playAnimation() : void
      {
         ...
         MovieClipUtil.playMc(this._animation,2,this._animation.totalFrames,function():void
         {
            ...
            GuideManager.instance.addGuide2Target(_loc1_,0,0,new Point(769,273),false,false,9);
      ...
      private function loadRes() : void
      {
         if(!this._resLib)
         {
            ...
            QueueLoader.load(URLUtil.getActivityAnimation("partyActivity/partyActivity"),"domain",this.onLoadComplete);
         }
      }
      ...
      {
         var _loc1_:Mobile = null;
         this._morph.removeEventListener("click",this.onMorph);
