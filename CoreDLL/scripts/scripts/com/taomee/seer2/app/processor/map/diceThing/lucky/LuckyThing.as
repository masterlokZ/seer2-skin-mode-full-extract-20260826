package com.taomee.seer2.app.processor.map.diceThing.lucky
{
   ...
   public class LuckyThing extends BaseDiceThing
   {
      ...
      override public function setUpThing() : void
      {
         if(thingInfo.isMee)
         {
            QueueLoader.load(URLUtil.getRes("map/dice/card/" + thingInfo.type + "Mee_" + thingInfo.id + ".swf"),"swf",this.onLoadeComplete);
         }
         else
         {
            QueueLoader.load(URLUtil.getRes("map/dice/card/" + thingInfo.type + "_" + thingInfo.id + ".swf"),"swf",this.onLoadeComplete);
         }
      }
      ...
      private function onLoadeComplete(param1:ContentInfo) : void
      {
         ...
         MovieClipUtil.playMc(this.cardMc,1,this.cardMc.totalFrames,function():void
         {
            ...
            okBtn = cardMc["okBtn"];
            okBtn.addEventListener("click",overThing);
         });
      }
      
      protected function overThing(param1:MouseEvent) : void
      {
         this.okBtn.removeEventListener("click",this.overThing);
