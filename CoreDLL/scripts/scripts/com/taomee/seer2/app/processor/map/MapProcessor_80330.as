   ...
   public class MapProcessor_80330 extends MapProcessor
   {
      ...
      private function onNpcClick(param1:MouseEvent) : void
      {
         ...
         else
         {
            ...
            NpcDialog.show(DIALOG_LIST[this._par.infoVec[3] - 1],PetConfig.getPetDefinition(NPC_LIST[this._par.infoVec[3] - 1]).name,[[0,"     战胜我 (" + [this._par.infoVec[0],this._par.infoVec[1],this._par.infoVec[2]][this._par.infoVec[3] - 1] + "/" + 3 + ") 次就可以获得觉醒，你今天还有" + leftNum.toString() + "次机会哦。"]],["开始挑战","我不觉了"],[function():void
            {
               if(leftNum > 0)
               {
               ...
               {
                  NpcDialog.show(DIALOG_LIST[_par.infoVec[3] - 1],PetConfig.getPetDefinition(NPC_LIST[_par.infoVec[3] - 1]).name,[[0,"     战胜我 (" + [_par.infoVec[0],_par.infoVec[1],_par.infoVec[2]][_par.infoVec[3] - 1] + "/" + 3 + ") 次就可以获得觉醒，你今天还有" + leftNum.toString() + "次机会哦。"]],["开始挑战","我不觉了"],[function():void
                  {
                     if(leftNum > 0)
                     {
                        FightManager.startFightWithWild(FIGHT_INDEX_LIST[_par.infoVec[3] - 1]);
