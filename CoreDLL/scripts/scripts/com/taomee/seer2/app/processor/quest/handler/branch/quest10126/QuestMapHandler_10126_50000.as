package com.taomee.seer2.app.processor.quest.handler.branch.quest10126
{
   ...
   public class QuestMapHandler_10126_50000 extends QuestMapHandler
   {
      ...
      override public function processMapComplete() : void
      {
         ...
         if(QuestManager.isStepComplete(_quest.id,3) && !QuestManager.isStepComplete(_quest.id,4) && DateUtil.isInTime(new Date(2012,8,23,23),new Date(2012,8,25,23)))
         {
            this.initStep4();
         }
      }
      ...
      private function initStep4() : void
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < _map.content.numChildren)
         {
            _loc1_ = _map.content.getChildAt(_loc2_);
            if(_loc1_ is HomeGarbage)
            {
               DisplayObjectUtil.removeFromParent(_loc1_);
            }
            _loc2_++;
         }
         this._resourceLoader = new ResourceLibraryLoader(URLUtil.getRes("common/home/garbage.swf"));
      ...
      private function handlerGarbage(param1:ResourceLibrary) : void
      {
         ...
         while(_loc2_ < this.garbageNum)
         {
            ...
            _loc3_.addEventListener("click",this.clearGarbage,false,1000);
            _loc2_++;
         }
      }
