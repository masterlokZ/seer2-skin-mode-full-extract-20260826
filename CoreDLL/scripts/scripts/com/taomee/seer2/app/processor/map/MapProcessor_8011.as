package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_8011 extends TitleMapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         this._castBtn.addEventListener("click",this.onCastClick);
         this._gameBtn.addEventListener("click",this.onGameClick);
         this._game2Btn.addEventListener("click",this.onGame2Click);
         this._devilTrainBtn.addEventListener("click",this.onDevilTrain);
         ...
         this._giftMachine.addEventListener("click",this.onGiftMachine);
         DialogPanel.addEventListener("customUnitClick",this.onCustomUnitClick);
         StatisticsManager.sendNovice("0x10033119");
         this.initVipPyramid();
         this.initDecoration();
         this._vipBack = _map.content["abc"];
         this._vipBack.addEventListener("click",this.onVipBack);
         this._newYearVip = _map.content["newYearVip"];
         this._newYearVip.addEventListener("click",this.onNewYearVip);
      }
      ...
      {
         SceneManager.changeScene(1,3817);
      }
      ...
      private function initOpen() : void
      {
         ...
         ServerBufferManager.updateServerBufferGroup(68,_loc2_);
      }
      ...
      private function onGiftMachine(param1:MouseEvent) : void
      {
         ...
         StatisticsManager.sendNovice("0x10033467");
      }
      ...
      private function initVipPyramid() : void
      {
         ...
         this._vipPyramidBtn.addEventListener("click",this.onVipPyramidBtn);
      }
      
      private function onVipPyramidBtn(param1:MouseEvent) : void
      {
         StatisticsManager.sendNovice("0x1003357F");
      ...
      private function onCustomUnitClick(param1:DialogPanelEvent) : void
      {
         ...
         {
            StatisticsManager.sendNoviceAccountHttpd("0x1003306f");
      ...
      private function checkIsComplete() : void
      {
         ...
         var _loc1_:uint = _loc2_.getDay() % 3;
         var _loc3_:Array = [[10059,10060,10061],[10062,10063,10064],[10065,10066,10067]];
         if(QuestManager.isComplete(10068) == false)
         {
            ModuleManager.toggleModule(URLUtil.getAppModule("JBCollectMaterialPanel"),"正在打开面板...",_loc1_);
         }
         else
         {
      ...
      private function onCastClick(param1:MouseEvent) : void
      {
         ...
         StatisticsManager.sendNovice("0x10033466");
      }
      
      private function onGameClick(param1:MouseEvent) : void
      {
         StatisticsManager.sendNovice("0x1003357C");
      ...
      {
         StatisticsManager.sendNovice("0x1003357B");
         ModuleManager.toggleModule(URLUtil.getAppModule("MiniGameIntroPanel"),"正在打开vip小游戏...",{
            "gameId":12,
            "isVip":1
         });
      ...
      {
         StatisticsManager.sendNovice("0x1003357D");
      ...
      override public function dispose() : void
      {
         ...
         this._castBtn.removeEventListener("click",this.onCastClick);
         this._gameBtn.removeEventListener("click",this.onGameClick);
         this._game2Btn.removeEventListener("click",this.onGame2Click);
         this._devilTrainBtn.removeEventListener("click",this.onDevilTrain);
