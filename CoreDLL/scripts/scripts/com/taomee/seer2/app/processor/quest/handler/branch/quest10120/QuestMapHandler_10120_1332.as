   ...
   public class QuestMapHandler_10120_1332 extends QuestMapHandler
   {
      ...
      private function startTalkI() : void
      {
         NpcDialog.show(NpcUtil.getSeerNpcId(),"我",[[0,"（前面的人依稀是船长，但是和辛迪船长的样子又有些不同）"]],["上去问问他"],[function():void
         {
            ...
            _captainer.addEventListener("click",startTalkII);
         }]);
      }
      ...
      private function playFullMovieI() : void
      {
         ...
         {
            QuestManager.addEventListener("stepComplete",finishStepI);
      ...
      private function finishStepI(param1:QuestEvent) : void
      {
         ...
         QuestManager.removeEventListener("stepComplete",this.finishStepI);
         ...
         if(SceneManager.prevSceneType == 2)
         {
            if(145 == fightIndex)
            {
               ...
               {
                  QuestManager.addEventListener("stepComplete",playSceneMc);
      ...
      {
         FightManager.startFightWithWild(145);
         FightManager.addEventListener("START_ERROR",this.onFightIStartError);
         FightManager.addEventListener("START_SUCCESS",this.onFightIStartOk);
      }
      
      private function onFightIStartOk(param1:FightStartEvent) : void
      {
         FightManager.removeEventListener("START_ERROR",this.onFightIStartError);
         FightManager.removeEventListener("START_SUCCESS",this.onFightIStartOk);
      }
      ...
      private function playSceneMc(param1:QuestEvent = null) : void
      {
         ...
         QuestManager.removeEventListener("stepComplete",this.playSceneMc);
      ...
      {
         FightManager.removeEventListener("START_ERROR",this.onFightStartError);
         FightManager.removeEventListener("START_SUCCESS",this.onFightStartOk);
      }
      ...
      {
         FightManager.startFightWithWild(146);
         FightManager.addEventListener("START_ERROR",this.onFightStartError);
         FightManager.addEventListener("START_SUCCESS",this.onFightStartOk);
      }
