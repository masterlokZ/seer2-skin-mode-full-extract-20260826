   ...
   public class HalloweenPumpkinAct
   {
         ...
         if(_loc2_.fullYear == 2012 && _loc2_.month == 9 && _loc2_.date == 27)
         {
            ...
            else if(SceneManager.active.mapID == ActorManager.actorInfo.id)
            {
               ...
               else
               {
                  ...
                  if(_loc1_ < 5 && SceneManager.prevSceneType != 2)
                  {
                     this.createPet();
                  }
               }
      ...
      private function updateTime(param1:int) : void
      {
         ...
         else
         {
            ...
            if(this.pet)
            {
               MobileManager.removeMobile(this.pet,"pet");
               this.pet.removeEventListener("click",this.toFight);
               this.pet = null;
            }
         }
      }
      ...
      private function checkTips() : void
      {
         ...
         {
            QueueLoader.load(URLUtil.getActivityAnimation("hitStickBar"),"swf",this.onComplete1);
         }
      }
      ...
      private function createPet() : void
      {
         ...
         this.pet.addEventListener("click",this.toFight,false,1000);
         MobileManager.addMobile(this.pet,"pet");
         QueueLoader.load(URLUtil.getActivityAnimation("hitStickBar"),"swf",this.onComplete);
      }
      ...
      private function onComplete(param1:ContentInfo) : void
      {
         ...
         MovieClipUtil.playMc(this.pumpkinPetAppear,1,this.pumpkinPetAppear.totalFrames,function():void
         {
            ...
            groundBar.addEventListener("click",getBar);
         },true);
      }
      ...
      private function getBar(param1:MouseEvent) : void
      {
         ...
         this.groundBar.removeEventListener("click",this.getBar);
         ...
         LayerManager.stage.addEventListener("click",this.hitPet);
      ...
      private function hitPet(param1:MouseEvent) : void
      {
         ...
         LayerManager.stage.removeEventListener("click",this.hitPet);
         MovieClipUtil.playMc(this.hitStickBar,1,this.hitStickBar.totalFrames,function():void
         {
            LayerManager.stage.addEventListener("click",hitPet);
            if(hitStickBar.hitTestObject(pet))
            {
               ...
               if(currentBlood == 0)
               {
                  StatisticsManager.sendNovice("0x1003332e");
                  ...
                  LayerManager.stage.removeEventListener("click",hitPet);
