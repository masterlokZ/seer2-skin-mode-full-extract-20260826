package com.taomee.seer2.app.actives
{
   ...
   public class TwelveTurnFightAct
   {
      ...
      private var seatTips:Array = [new Point(295,205),new Point(630,300),new Point(630,300)];
      ...
      public function dispose() : void
      {
         ...
         SeatTipsManager.removeSeat(this.seatTips[0],80068);
         SeatTipsManager.removeSeat(this.seatTips[1],80068);
         SeatTipsManager.removeSeat(this.seatTips[2],80068);
      }
      ...
      public function setup(param1:MapModel) : void
      {
         ...
         if(this.timeDate.fullYear != 2013 || this.timeDate.month != 12 - 1 || !_loc2_ || this.timeDate.hours != 13)
         {
            return;
         }
         ...
         if(!this.resLib)
         {
            QueueLoader.load(URLUtil.getActivityAnimation("TwelveTurnFight"),"domain",this.onLoadComplete);
         }
         else
         {
            DayLimitManager.getDoCount(877,this.getState);
         }
         ...
         if(!BitUtil.getBit(this.fightState,this.currentTurn - 1))
         {
            ...
            MovieClipUtil.playMc(this.sceneMc,1,this.sceneMc.totalFrames,function():void
            {
               if(currentNpc)
               {
                  ...
                  currentNpc.removeEventListener("click",toFight);
               }
               sceneMc.stop();
               NpcDialog.show(npcID,npcName,[[0,diaString]],[okString],[function():void
               {
                  ...
                  currentNpc.addEventListener("click",toFight);
                  SeatTipsManager.registerSeat(seatTips[actIndex],80068);
               }]);
            },true);
         }
         else
      ...
      protected function toFight(param1:MouseEvent) : void
      {
         if(BitUtil.getBit(this.fightState,this.currentTurn - 1))
         {
            if(this.currentTurn % 2 != 0)
            {
               NpcDialog.show(644,"电魂巴图",[[0,"每5分钟可以挑战我们其中一个。紫电，如今的我是不会被你战胜的！"]],["那你可得加油咯！"]);
            }
            else
            {
               NpcDialog.show(592,"紫电",[[0,"每5分钟可以挑战我们其中一个。为了尊严而战，我才是帝国最强电系精灵！"]],["你的话说的太早了！"]);
            }
            return;
         }
         SeatTipsManager.removeSeat(this.seatTips[this.actIndex],80068);
         FightManager.addEventListener("FIGHT_OVER",this.fightOver);
         FightManager.startFightWithWild(this.FIGHT_LIST[this.actIndex]);
      }
      
      protected function fightOver(param1:Event) : void
      {
         FightManager.removeEventListener("FIGHT_OVER",this.fightOver);
         if(this.currentTurn % 2 != 0)
         {
            NpcDialog.show(644,"电魂巴图",[[0,"实力不错嘛！看样子你是一个可敬的对手！"]],["太棒啦！"],[this.checkTime]);
         }
         else if(this.currentTurn % 2 == 0 && this.currentTurn != 12)
         {
            NpcDialog.show(592,"紫电",[[0,"挺厉害嘛！不能太小看你了。"]],["你也不错！"],[this.checkTime]);
         }
         else
         {
            NpcDialog.show(592,"紫电",[[0,"实力不错嘛！看样子你是一个可敬的对手！"]],["太棒啦！"],[this.completeAct]);
         }
      }
