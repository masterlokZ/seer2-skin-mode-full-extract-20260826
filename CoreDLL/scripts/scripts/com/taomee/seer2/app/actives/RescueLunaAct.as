package com.taomee.seer2.app.actives
{
   ...
   public class RescueLunaAct
   {
      ...
      private var positions:Array = [new Point(255,265),new Point(515,405),new Point(670,185)];
      
      private var bloodPositions:Array = [new Point(0,0),new Point(0,-200),new Point(0,-120)];
      ...
      public function setup() : void
      {
         ...
         DayLimitManager.getDoCount(670,this.getShootState);
      }
      ...
      public function dispose() : void
      {
         ...
         {
            this.panGuanMobile.removeEventListener("click",this.showNpcDia);
         }
      ...
      private function initMobile() : void
      {
         ...
         {
            _loc1_ = new Mobile();
            _loc1_.resourceUrl = URLUtil.getNpcSwf(this.npcIDs[_loc2_]);
            _loc1_.x = this.positions[_loc2_].x;
            _loc1_.y = this.positions[_loc2_].y;
            _loc1_.visible = false;
            this.npcList.push(_loc1_);
            MobileManager.addMobile(_loc1_,"npc");
            this.bloodBarMc.push(this._map.libManager.getMovieClip("BloodBarMc"));
            this.bloodBarMc[_loc2_].x = this.bloodPositions[_loc2_].x - this.bloodBarMc[_loc2_].width / 2;
            this.bloodBarMc[_loc2_].y = this.bloodPositions[_loc2_].y;
            _loc1_.addChild(this.bloodBarMc[_loc2_]);
            _loc2_++;
         }
      }
      ...
      private function startShoot() : void
      {
         ...
         MovieClipUtil.playMc(this.shootTips,1,this.shootTips.totalFrames,function():void
         {
            ...
            ShootController.addEventListener("playEnd",chekTarget);
      ...
      private function setPan() : void
      {
         ...
         this.panGuanMobile.addEventListener("click",this.showNpcDia);
         MobileManager.addMobile(this.panGuanMobile,"npc");
      }
      ...
      private function updateTime(param1:int) : void
      {
         ...
         ++this.overTime;
         this.leftTime = 180 - this.overTime;
      ...
      private function chekTarget(param1:ShootEvent) : void
      {
         ...
         if(_loc2_.userID == ActorManager.actorInfo.id && this.currentNpc.hitTestPoint(_loc2_.endPos.x,_loc2_.endPos.y))
         {
            --this.currentBlood;
      ...
      private function deadMobile() : void
      {
         ...
         else if(this.waveNum > 2)
         {
            ...
            while(_loc1_ < 3)
            {
               MobileManager.removeMobile(this.npcList[_loc1_],"npc");
               _loc1_++;
            }
            this.cretatePan();
         }
      ...
      private function resetNpc() : void
      {
         ...
         MobileManager.addMobile(this.npcList[0],"npc");
         MobileManager.addMobile(this.npcList[1],"npc");
         MobileManager.addMobile(this.npcList[2],"npc");
         this.currentNpc = this.npcList[this.currentIndex];
