package com.taomee.seer2.app.controls
{
   ...
   public class ProcessBuddyPanel extends Sprite
   {
      ...
      {
         this._userIdTxt.addEventListener("focusIn",this.onUserIdFocusIn);
         this._userIdTxt.addEventListener("focusOut",this.onUserIdFocusOut);
         this._confirmBtn.addEventListener("click",this.onConfirmClick);
         this._cancelBtn.addEventListener("click",this.onCancelClick);
      }
      ...
      {
         var _loc3_:uint = 0;
         var _loc2_:String = StringUtil.trim(this._userIdTxt.text);
      ...
      private function verifyAddBuddy(param1:uint) : Boolean
      {
         ...
         else if(UserGroupManager.containsUser("buddy",param1))
         {
      ...
      private function verifyRemoveBuddy(param1:uint) : Boolean
      {
         ...
         else if(UserGroupManager.containsUser("buddy",param1) == false)
         {
            AlertManager.showAlert("好友列表中找不到该用户");
            _loc2_ = false;
         }
      ...
      private function verifyAddBlack(param1:uint) : Boolean
      {
         ...
         else if(UserGroupManager.containsUser("black",param1))
         {
      ...
      {
         this._userIdTxt.removeEventListener("focusIn",this.onUserIdFocusIn);
         this._userIdTxt.removeEventListener("focusOut",this.onUserIdFocusOut);
         this._confirmBtn.removeEventListener("click",this.onConfirmClick);
         this._cancelBtn.removeEventListener("click",this.onCancelClick);
