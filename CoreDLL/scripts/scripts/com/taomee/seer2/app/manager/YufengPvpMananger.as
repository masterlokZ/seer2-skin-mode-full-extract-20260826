package com.taomee.seer2.app.manager
{
   ...
   public class YufengPvpMananger
   {
      ...
      private static function matchSuccess(param1:MessageEvent) : void
      {
         ...
         userInfoCount = data.readUnsignedInt();
         thisPetList = Vector.<PetInfo>([]);
         remotePetList = Vector.<PetInfo>([]);
         thisInfo = new UserInfo();
         remoteInfo = new UserInfo();
         for(i = 0; i < userInfoCount; )
         {
            id = data.readUnsignedInt();
            trainerIntegral = data.readUnsignedInt();
            petInfoCount = data.readUnsignedInt();
            if(id != ActorManager.actorInfo.id)
            {
               remoteId = id;
            }
            for(j = 0; j < petInfoCount; )
            {
               petID = data.readUnsignedInt();
               petLevel = data.readUnsignedInt();
               petTime = data.readUnsignedInt();
               petInfo = new PetInfo();
            ...
            if(id == ActorManager.actorInfo.id)
            {
               ...
               thisEquipCount = data.readUnsignedInt();
               for(m = 0; m < thisEquipCount; )
               {
            ...
            else
            {
               ...
               remoteEquipCount = data.readUnsignedInt();
               for(n = 0; n < remoteEquipCount; )
               {
                  equipItem = new EquipItem(data.readUnsignedInt());
            ...
            meleeCount = data.readUnsignedInt();
            for(k = 0; k < meleeCount; )
            {
         ...
         {
            var _loc2_:PetInfo = thisPetList[param1];
