package com.taomee.seer2.app.pet.data
{
   internal class PetSkillInfo
   {
      ...
      public function hasSkillInfo(param1:SkillInfo) : Boolean
      {
         ...
         var _loc4_:uint = this._skillInfoVec.length;
         var _loc3_:uint = 0;
         while(_loc3_ < _loc4_)
         {
            if(this._skillInfoVec[_loc3_].id == param1.id)
            {
               _loc2_ = true;
               break;
            }
      ...
      {
         var _loc2_:Boolean = false;
         var _loc4_:uint = this._candidateSkillInfoVec.length;
         var _loc3_:uint = 0;
         while(_loc3_ < _loc4_)
         {
            if(this._candidateSkillInfoVec[_loc3_].id == param1.id)
            {
               _loc2_ = true;
               break;
            }
