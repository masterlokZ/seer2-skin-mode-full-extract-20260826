package com.taomee.seer2.app.animationInteractive
{
   ...
   public class LeaveAlone extends BaseAniamationInteractive
   {
      ...
      override protected function paramAnimation() : void
      {
         ...
         while(_loc2_ < 17)
         {
            _loc4_ = _animation["treeBtn_" + _loc2_];
            _loc4_.buttonMode = true;
            _loc4_.addEventListener("click",this.onTreeBtnClick);
            this._treeBtnVec.push(_loc4_);
            _loc2_++;
         }
         var _loc1_:int = 0;
         while(_loc1_ < 7)
         {
            _loc3_ = _animation["animation_" + _loc1_];
            this._animationVec.push(_loc3_);
            _loc1_++;
         }
         _animation.addEventListener("enterFrame",this.onEnterFramer);
      ...
      private function createAnimationLines() : void
      {
         ...
         while(_loc3_ < 7 - 1)
         {
            ...
            while(_loc2_ < 7)
            {
               _loc5_ = new Point(this._animationVec[_loc3_].x,this._animationVec[_loc3_].y);
               _loc4_ = new Point(this._animationVec[_loc2_].x,this._animationVec[_loc2_].y);
               _loc1_ = new Line(_loc5_,_loc4_);
               this._animationLineVec.push(_loc1_);
               _loc2_++;
            }
            _loc3_++;
         }
      ...
      private function onTreeBtnClick(param1:MouseEvent) : void
      {
         ...
         {
            if(_loc4_.x == this._startPos.x && _loc4_.y == this._startPos.y)
            {
               return;
            }
            _loc3_ = new Line(this._startPos,_loc4_);
            this._treeLineVec.push(_loc3_);
            ++this._lineCount;
      ...
      {
         var _loc2_:Point = new Point(LayerManager.topLayer.mouseX - 120,LayerManager.topLayer.mouseY - 50);
         var _loc1_:Number = this._startPos.x - _loc2_.x;
         var _loc4_:Number = this._startPos.y - _loc2_.y;
         var _loc3_:Number = Math.sqrt(_loc1_ * _loc1_ + _loc4_ * _loc4_);
         this._currentLine.scaleX = _loc3_ / this._lineLength;
         this._currentLine.rotation = Math.atan2(_loc4_,_loc1_) * 180 / 3.141592653589793 + 180;
      }
      ...
      private function isCross(param1:Line, param2:Line) : Boolean
      {
         ...
         if(param1.k == 2147483647)
         {
            _loc4_ = param1.b;
            _loc3_ = param2.k * param1.b + param2.b;
         }
         else
         {
            _loc4_ = (param2.b - param1.b) / (param1.k - param2.k);
            _loc3_ = param1.b - param1.k * (param1.b - param2.b) / (param1.k - param2.k);
         }
         if(_loc4_ >= param1.minX && _loc4_ <= param1.maxX && _loc3_ >= param1.minY && _loc3_ <= param1.maxY)
         {
            if(_loc4_ >= param2.minX && _loc4_ <= param2.maxX && _loc3_ >= param2.minY && _loc3_ <= param2.maxY)
            {
               return true;
            }
         }
      ...
      private function checkAnswer() : void
      {
         if(this.isAllAlone())
         {
            ...
            {
               clearTimeout(_delay);
               dispatchEvent(new Event("leaveAlone"));
               dispose();
            },2000);
         }
         else
      ...
      private function reset() : void
      {
         ...
         this._currentLine = null;
         this._lineCount = 0;
         var _loc2_:int = int(this._treeLineVec.length);
         var _loc1_:int = 0;
         while(_loc1_ < _loc2_)
         {
      ...
      private function clearEvent() : void
      {
         ...
         while(_loc1_ < 17)
         {
            this._treeBtnVec[_loc1_].removeEventListener("click",this.onTreeBtnClick);
            _loc1_++;
         }
         _animation.removeEventListener("enterFrame",this.onEnterFramer);
      }
