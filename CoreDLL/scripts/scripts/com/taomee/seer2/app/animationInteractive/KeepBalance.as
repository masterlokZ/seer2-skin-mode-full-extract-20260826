package com.taomee.seer2.app.animationInteractive
{
   ...
   public class KeepBalance extends BaseAniamationInteractive
   {
      ...
      override protected function paramAnimation() : void
      {
         ...
         this._actorMC.addEventListener("enterFrame",this.onActorEnterFramer);
      }
      ...
      private function initTimers() : void
      {
         ...
         this._dTimer.addEventListener("timer",this.onDTimer);
         this._randomTimer = new Timer(1000);
         this._randomTimer.addEventListener("timer",this.onRandomTimer);
      }
      ...
      private function startGame() : void
      {
         ...
         _animation.addEventListener("keyDown",this.onKeyDown);
         _animation.addEventListener("keyUp",this.onKeyUp);
      ...
      {
         super.hide();
         this._actorMC.removeEventListener("enterFrame",this.onActorEnterFramer);
         this.closeKeyEventListener();
         this.clearTimers();
      ...
      private function clearTimers() : void
      {
         ...
         this._dTimer.stop();
         this._dTimer.removeEventListener("timer",this.onDTimer);
         this._randomTimer.stop();
         this._randomTimer.removeEventListener("timer",this.onRandomTimer);
      }
      
      private function closeKeyEventListener() : void
      {
         _animation.removeEventListener("keyDown",this.onKeyDown);
         _animation.removeEventListener("keyUp",this.onKeyUp);
      }
   }
}
