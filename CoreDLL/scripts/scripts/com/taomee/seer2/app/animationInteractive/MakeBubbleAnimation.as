package com.taomee.seer2.app.animationInteractive
{
   ...
   public class MakeBubbleAnimation extends BaseAniamationInteractive
   {
      ...
      {
         _animation.addEventListener("mouseDown",this.onMouseDown);
         _animation.addEventListener("mouseUp",this.onMouseUp);
      }
      
      private function closeEventListener() : void
      {
         _animation.removeEventListener("mouseDown",this.onMouseDown);
         _animation.removeEventListener("mouseUp",this.onMouseUp);
      }
      ...
      private function judgeHitBubble(param1:MovieClip, param2:Point, param3:Number) : Boolean
      {
         ...
         if(Math.sqrt(_loc5_ * _loc5_ + _loc4_ * _loc4_) <= param3)
         {
            return true;
         }
         return false;
      ...
      {
         var _loc2_:int = Math.min(this._currentBubble.x - this.LEFT_EDGE.x,this._currentBubble.y - this.LEFT_EDGE.y);
         if(_loc2_ < this._currentBubble.width / 2)
         {
            this._timer.stop();
         }
         var _loc1_:int = Math.min(this.RIGHT_EDGE.x - this._currentBubble.x,this.RIGHT_EDGE.y - this._currentBubble.y);
      ...
      private function updataTxt() : void
      {
         ...
         if(this._bubbleNum == 10)
         {
            ...
            {
               dispatchEvent(new Event("makeBubble"));
               this.dispose();
            }
            else
            {
      ...
      private function computeArea() : void
      {
         ...
         while(_loc1_ < this._bubbleVec.length)
         {
            _loc3_ = this._bubbleVec[_loc1_].width / 2;
            _loc2_ += 3.141592653589793 * _loc3_ * _loc3_ * 100;
            _loc1_++;
         }
         this._percentArea = Math.round(_loc2_ / 187240);
      }
