package com.taomee.seer2.app.gameRule.data
{
   ...
   public class PetCanvas
   {
      ...
      {
         if(this.cureBtn.hasEventListener("click"))
         {
            this.cureBtn.removeEventListener("click",this.onCureBtnClick);
         }
      }
      
      public function addCureListener() : void
      {
         this.cureBtn.addEventListener("click",this.onCureBtnClick);
      }
      ...
      public function updatePetInfo() : void
      {
         ...
         nn = nn > 1 ? 1 : nn;
         this.HP.scaleX = nn;
         url = URLUtil.getPetIcon(this.petInfo.resourceId);
      ...
      public function updateEvolve() : void
      {
         ...
         {
            _loc2_ = "";
            _loc1_ = this.petInfo.evolveLevel > 1000 ? uint(this.petInfo.evolveLevel - 1000) : this.petInfo.evolveLevel;
            if(_loc1_ > 2)
            {
               if(_loc1_ < 5)
               {
               ...
               {
                  _loc2_ = "UI_GoldBorder";
               }
               else
               {
                  _loc2_ = "UI_PurpleBorder";
               }
               if(_loc2_.length > 0)
               {
                  this._border = UIManager.getMovieClip(_loc2_);
                  this._border.x = 0;
                  this._border.y = 0;
      ...
      private function onCureBtnClick(param1:MouseEvent) : void
      {
         ...
         {
            PetInfoManager.removeEventListener("petCure",onPetCureSuccess);
            DisplayObjectUtil.enableButton(cureBtn);
            ui.filters = [];
         ...
         if(this._petInfo != null)
         {
            ...
            {
               PetInfoManager.addEventListener("petCure",onPetCureSuccess);
