package com.taomee.seer2.app.gameRule.spt
{
   ...
   public class SptKadoshSupport extends BaseSptSupport
   {
      ...
      override public function init(param1:MapModel) : void
      {
         ...
         _sptBufferPosition = 10;
         super.init(param1);
      }
      
      override protected function fightFailure(param1:LittleEndianByteArray) : void
      {
         var _loc2_:uint = Math.random() * 1000 % Weak_Point_Vec.length;
         var _loc4_:XML = new XML(SptDialogConfig.getSptDialog(_sptBossId,Weak_Point_Vec[_loc2_]));
         var _loc3_:DialogDefinition = new DialogDefinition(_loc4_);
      ...
      private function startTimer() : void
      {
         ...
         this._timer.addEventListener("timer",this.stopMcHandler);
         this._timer.start();
      }
      ...
      private function stopMcHandler(param1:TimerEvent) : void
      {
         this.showSPT("normal");
         this._timer.reset();
         this._timer.removeEventListener("timer",this.stopMcHandler);
         this._timer = null;
      }
      ...
      {
         var _loc2_:XML = new XML(SptDialogConfig.getSptDialog(_sptBossId,1));
