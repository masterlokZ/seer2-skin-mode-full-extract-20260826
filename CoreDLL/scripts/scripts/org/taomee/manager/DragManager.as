package org.taomee.manager
{
   ...
   public class DragManager
   {
      ...
      {
         (param1.target as InteractiveObject).dispatchEvent(new DragEvent("dragEnter",_dragInitiator,_dropTarget,_dragSource,_dropAction,_allowMove));
      }
      ...
      public static function doDrag(param1:InteractiveObject, param2:DragSource, param3:BitmapData = null, param4:Point = null, param5:Boolean = true) : void
      {
         ...
         _allowMove = param5;
         TaomeeManager.stage.addEventListener("mouseOver",onStageOver);
         TaomeeManager.stage.addEventListener("mouseOut",onStageOut);
         TaomeeManager.stage.addEventListener("mouseMove",onStageMove);
         TaomeeManager.stage.addEventListener("mouseUp",onStageUp);
         _dragInitiator.dispatchEvent(new DragEvent("dragStart",_dragInitiator,_dropTarget,_dragSource,_dropAction,_allowMove));
      }
      ...
      {
         (param1.target as InteractiveObject).dispatchEvent(new DragEvent("dragExit",_dragInitiator,_dropTarget,_dragSource,_dropAction,_allowMove));
      }
      
      public static function endDrag() : void
      {
         _isDragging = false;
         TaomeeManager.stage.removeEventListener("mouseOver",onStageOver);
         TaomeeManager.stage.removeEventListener("mouseOut",onStageOut);
         TaomeeManager.stage.removeEventListener("mouseMove",onStageMove);
         TaomeeManager.stage.removeEventListener("mouseUp",onStageUp);
         if(_dragInitiator)
         {
            _dragInitiator.dispatchEvent(new DragEvent("dragComplete",_dragInitiator,_dropTarget,_dragSource,_dropAction,_allowMove));
         }
         if(Boolean(_dropTarget) && Boolean(_mouseTarget))
         {
            if(_dropTarget == _mouseTarget)
            {
               _dropTarget.dispatchEvent(new DragEvent("dragDrop",_dragInitiator,_dropTarget,_dragSource,_dropAction,_allowMove));
            }
         }
         clear();
      }
      ...
      {
         upDateImageMove();
         _dragInitiator.dispatchEvent(new DragEvent("dragUpdate",_dragInitiator,_dropTarget,_dragSource,_dropAction,_allowMove));
         _mouseTarget = param1.target as InteractiveObject;
         _mouseTarget.dispatchEvent(new DragEvent("dragOver",_dragInitiator,_dropTarget,_dragSource,_dropAction,_allowMove));
      }
   }
}
