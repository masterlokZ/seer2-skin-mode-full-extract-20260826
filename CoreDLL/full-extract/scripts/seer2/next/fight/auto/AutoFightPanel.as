package seer2.next.fight.auto
{
   ...
   public class AutoFightPanel extends Sprite
   {
      ...
      private function initset() : void
      {
         ...
         yanshuBtn.addEventListener("click",this.onYanshu);
         cureBtn.addEventListener("click",this.onCure);
         helpBtn.addEventListener("click",this.onHelp);
         monitorBtn.addEventListener("click",this.onMonitor);
      ...
      private function getNode() : *
      {
         ...
         currentChar = text_.charAt(index);
         if(currentChar != "")
         {
      ...
      private function onStart(event:MouseEvent) : void
      {
         ...
         var reStart:Function = function(event:TimerEvent):void
         {
            if(!isRunning)
            {
               checkStartTimer.removeEventListener("timer",reStart);
         ...
         if(h != null)
         {
            ...
            checkStartTimer.addEventListener("timer",reStart);
            ...
            isRunning = true;
            this.opArray = [];
            FightManager.addEventListener("START_ERROR",this.FightError);
            FightOverEvent.addEventListener("fightOverEvent",this.continueFight);
            return;
         }
      ...
      private function continueFight(event:Event) : void
      {
         ...
         {
            delayTimer.removeEventListener("timer",cure);
            delayTimer.reset();
         ...
         FightOverEvent.removeEventListener("fightOverEvent",this.continueFight);
         if(this.fightTime == 0)
         {
            txtOut3.text += "[" + new Date().toLocaleTimeString() + "]" + "结束对战\n";
            endBtn.dispatchEvent(new MouseEvent("click"));
            return;
         }
         if(isRunning)
         {
            txtOut3.text += "[" + new Date().toLocaleTimeString() + "]" + "继续对战,还有" + this.fightTime + "次战斗\n";
            this.delayTimer.addEventListener("timer",cure);
            delayTimer.start();
         }
      }
      ...
      private function FightError(event:FightStartEvent) : void
      {
         ...
         FightManager.removeEventListener("START_ERROR",this.FightError);
         this.endBtn.dispatchEvent(new MouseEvent("click"));
      }
      ...
      {
         FightManager.startFightWithNPC(param1);
      }
      
      private function FightWithSPTHandler(param1:uint) : void
      {
         FightManager.startFightWithSPTBoss(param1);
      }
