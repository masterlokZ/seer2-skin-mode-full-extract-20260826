package com.taomee.seer2.app.swap
{
   ...
   public class SwapManager
   {
      ...
      private static function onGetReward(param1:MessageEvent) : void
      {
         ...
         {
            _loc4_ = _successHandler;
            _successHandler = null;
            _loc4_(_loc2_);
         }
         else
         {
            _loc3_ = new SwapInfo(_loc2_);
         }
      ...
      {
         SwapEventDispatcher.addEventListener("numberLimit",onNumberLimit);
         SwapEventDispatcher.addEventListener("swapEnd",onSwapEnd);
      }
      ...
      {
         var _loc3_:SwapItemIn = null;
         var _loc2_:SwapItem = SwapConfig.getSwapItem(param1);
         var _loc4_:Vector.<SwapItemIn> = _loc2_.itemInList;
         for each(_loc3_ in _loc4_)
         {
            if(ItemManager.getItemQuantityByReferenceId(_loc3_.referenceId) < _loc3_.count)
            {
               return false;
            }
         }
      ...
      {
         SwapEventDispatcher.removeEventListener("numberLimit",onNumberLimit);
         SwapEventDispatcher.removeEventListener("swapEnd",onSwapEnd);
      }
