package com.taomee.seer2.app.activity.processor.giftFromSeer
{
   ...
   public class GiftFromSeerMapHandler extends ActivityMapHandler
   {
      ...
      {
         if(SceneManager.active && SceneManager.prevSceneType == 2 && FightManager.currentFightRecord && FightManager.currentFightRecord.initData && Boolean(FightManager.currentFightRecord.initData.positionIndex) && FightManager.currentFightRecord.initData.positionIndex == 52)
         {
      ...
      private function showSpecial() : void
      {
         ...
         Connection.send(CommandSet.DAILY_LIMIT_1065,326);
      }
      
      private function isShowMiKa() : Boolean
      {
         return this._boxCount == 22 - 1;
      }
      ...
      private function showMiKa() : void
      {
         ...
         if(this._miKa == null)
         {
            QueueLoader.load(URLUtil.getActivityAnimation("giftFromSeer/miKa"),"swf",this.onMiKaLoaded);
         }
         else
         {
            this.playMiKa();
      ...
      private function playMiKa() : void
      {
         ...
         {
            clearMiKa();
            FightManager.startFightWithWild(52);
         },true);
      }
      ...
      private function showOpenBox() : void
      {
         ...
         {
            QueueLoader.load(URLUtil.getActivityAnimation("giftFromSeer/openBox"),"swf",this.onOpenBoxLoaded);
         }
         else
         {
            this.playOpenBox();
      ...
      private function playOpenBox() : void
      {
         ...
         MovieClipUtil.playMc(this._openBox,1,this._openBox.totalFrames,function():void
         {
            ...
            {
               _loc2_ = (_activityProcessor as GiftFromSeer).getRelateMapIDArr();
               if(_loc2_)
               {
                  _loc1_ = uint(_loc2_.indexOf(SceneManager.active.mapID));
                  if(_loc1_ != -1)
                  {
                     (_activityProcessor as GiftFromSeer).coldTime[_loc1_] = 60;
                  }
                  SwapManager.swapItem(219);
               }
            }
         },true);
      }
      ...
      protected function showBox() : void
      {
         ...
         if(_loc1_ != -1 && (_activityProcessor as GiftFromSeer).coldTime[_loc1_] == 0)
         {
            ...
            if(this._box == null)
            {
               QueueLoader.load(URLUtil.getActivityAnimation("giftFromSeer/box"),"swf",this.onBoxLoaded);
            }
            else
            {
               this.addBox();
      ...
      private function addBox() : void
      {
         if(Boolean(SceneManager.active) && Boolean(SceneManager.active.mapModel))
         {
            ...
            this._box.addEventListener("click",this.onBoxClick);
         }
      }
      ...
      private function clearBox() : void
      {
         ...
         if(this._box)
         {
            this._box.removeEventListener("click",this.onBoxClick);
