   ...
   public class FlySeriesFight extends ActivityProcessor
   {
      ...
      override protected function onMapComplete(param1:SceneEvent) : void
      {
         ...
         if(Boolean(SceneManager.active) && SceneManager.active.mapID == 770)
         {
            ...
            if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord && FightManager.currentFightRecord.initData && FightManager.currentFightRecord.initData.positionIndex && FightManager.currentFightRecord.initData.positionIndex == 50 && FightManager.fightWinnerSide != 1)
            {
               NpcDialog.show(80,"葛蕾芬兵团长",[[3,"放弃吧！葛蕾芬兵团是不可战胜的！"]],["哎……那我还是走了","继续挑战！"],[null,function():void
               {
                  FightManager.startFightWithWild(50,null,null,btnData);
               }]);
            }
            else if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord && FightManager.currentFightRecord.initData && FightManager.currentFightRecord.initData.positionIndex && FightManager.currentFightRecord.initData.positionIndex == 56 && FightManager.fightWinnerSide != 1)
            {
               NpcDialog.show(80,"葛蕾芬兵团长",[[3,"再来多少次结果都是一样的！"]],["哎……那我还是走了","继续挑战！"],[null,function():void
               {
                  FightManager.startFightBinaryWild(56,null,null,btnData);
               }]);
            }
      ...
      private function onEventStart(param1:MessageEvent) : void
      {
         ...
         if(parser.id == 14 && parser.index == 14)
         {
            NpcDialog.show(461,"受伤的葛蕾芬",[[0,"我的出生就是为葛蕾芬兵团获取胜利的荣光！既然不能完成这个使命……那就杀了我吧！"]],["杀戮不能解决任何问题！"],[function():void
            {
               ...
               {
                  FightManager.startFightWithWild(51);
               }]);
            }]);
         }
      }
      ...
      private function playSceneAnimation() : void
      {
         ...
         {
            QueueLoader.load(URLUtil.getActivityAnimation("flySeriesFight/1"),"swf",this.onSceneAnimationLoaded);
         }
         else
         {
      ...
      private function startFightAfterAnimation() : void
      {
         ...
         if(this._isDualPet)
         {
            FightManager.startFightBinaryWild(56,null,null,_loc1_);
         }
         else if(this._isSuper)
         {
            FightManager.startFightBinaryWild(1833,null,null,_loc1_);
         }
         else
         {
            FightManager.startFightWithWild(50,null,null,_loc1_);
         }
