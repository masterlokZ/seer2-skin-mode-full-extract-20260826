package com.taomee.seer2.app.activity.processor.salungInvade
{
   ...
   public class SalungSoldierBase extends Sprite
   {
      ...
      protected function loadRes(param1:String) : void
      {
         if(!this._info)
         {
            QueueLoader.load(URLUtil.getActivityAnimation(param1),"swf",this.onResLoaded);
         }
         else
         {
            this.onResLoaded();
      ...
      protected function initNpc() : void
      {
         ...
         this._npc.addEventListener("click",this.showHelpDialog);
         this._npc.x = MAP_OOSITION["m_" + this._mapId][0];
         this._npc.y = MAP_OOSITION["m_" + this._mapId][1];
         SceneManager.addEventListener("switchComplete",this.removeNpc);
      }
      ...
      protected function showHelpDialog(param1:MouseEvent = null) : void
      {
         ...
         this._helpMc["beginBtn"].addEventListener("click",this.beginGame);
         this._helpMc["closeBtn"].addEventListener("click",this.giveUpGame);
      }
      
      private function beginGame(param1:MouseEvent) : void
      {
         this._helpMc["beginBtn"].removeEventListener("click",this.beginGame);
         this._helpMc["closeBtn"].removeEventListener("click",this.beginGame);
      ...
      {
         SceneManager.removeEventListener("switchComplete",this.removeNpc);
         DisplayUtil.removeForParent(this._npc);
      }
