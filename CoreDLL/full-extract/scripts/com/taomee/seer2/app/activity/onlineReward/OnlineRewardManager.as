package com.taomee.seer2.app.activity.onlineReward
{
   ...
   public class OnlineRewardManager
   {
      ...
      private static function onMapSwitchComplete(param1:SceneEvent) : void
      {
         ...
         {
            QueueLoader.load(URLUtil.getActivityAnimation("onlineReward"),"swf",onResLoaded);
         }
      }
      ...
      private static function initRewardToggle(param1:ContentInfo) : void
      {
         ...
         _toggleMc["hotArea"].addEventListener("click",onToggleClick);
         _toggleMc["hotArea"].addEventListener("mouseOver",onToggleOver);
         _toggleMc.addEventListener("enterFrame",onToggleEnterFrame);
         _toggleMc.gotoAndStop(1);
      ...
      private static function onToggleEnterFrame(param1:Event) : void
      {
         ...
         {
            _toggleMc.gotoAndStop(1);
            _toggleMc.removeEventListener("enterFrame",onToggleEnterFrame);
            return;
         }
      ...
      private static function onToggleClick(param1:MouseEvent) : void
      {
         ...
         StatisticsManager.sendNovice("0x10033001");
         var _loc2_:Object = {};
      ...
      {
         _toggleMc["hotArea"].removeEventListener("mouseOver",onToggleOver);
         if(_isGetAll)
         {
            return;
         }
         _toggleMc["hotArea"].addEventListener("mouseOut",onToggleOut);
      ...
      private static function updateTime() : void
      {
         ...
         (_timeMC["mcMinDecade"] as MovieClip).gotoAndStop(uint(_loc2_ / 10) + 1);
         (_timeMC["mcMinUnit"] as MovieClip).gotoAndStop(uint(_loc2_ % 10) + 1);
         (_timeMC["mcSecDecade"] as MovieClip).gotoAndStop(uint(_loc1_ / 10) + 1);
         (_timeMC["mcSecUnit"] as MovieClip).gotoAndStop(uint(_loc1_ % 10) + 1);
      }
      
      private static function onTick(param1:uint) : void
      {
         if(Nono1122Info.instance().resideTime == 0 && SceneManager.active.type != 2)
         {
            Tick.instance.removeRender(onTick);
            Connection.send(CommandSet.GET_ONLINE_CAN_SWAP_NUM_1122);
         }
