package com.taomee.seer2.app.activity.processor
{
   ...
   public class FightSaBoEr extends ActivityProcessor
   {
      ...
      override protected function onMapComplete(param1:SceneEvent) : void
      {
         ...
         if(Boolean(SceneManager.active) && SceneManager.active.mapID == 660)
         {
            btnData = new ButtonPanelData();
            btnData.catchEnabled = false;
            if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord && FightManager.currentFightRecord.initData && FightManager.currentFightRecord.initData.positionIndex && FightManager.currentFightRecord.initData.positionIndex == 61 && FightManager.fightWinnerSide != 1)
            {
               NpcDialog.show(83,"萨伯尔兵团长",[[1,"如何？萨伯尔兵团和那些看门的可不一样……（露出了嘲讽的表情）"]],["哎……那我还是走了","继续挑战！"],[null,function():void
               {
                  FightManager.startFightWithWild(61,null,null,btnData);
               }]);
            }
            else if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord && FightManager.currentFightRecord.initData && FightManager.currentFightRecord.initData.positionIndex && FightManager.currentFightRecord.initData.positionIndex == 60 && FightManager.fightWinnerSide != 1)
            {
               NpcDialog.show(83,"萨伯尔兵团长",[[1,"如何？萨伯尔兵团和那些看门的可不一样……（露出了嘲讽的表情）"]],["哎……那我还是走了","继续挑战！"],[null,function():void
               {
                  FightManager.startFightBinaryWild(60,null,null,btnData);
               }]);
            }
      ...
      private function onEventStart(param1:MessageEvent) : void
      {
         ...
         if(parser.id == 17 && parser.index == 16)
         {
            NpcDialog.show(463,"萨伯尔",[[0,"您……是我遇见过数一数二的高手！不愧是总督大人特许进入飞翼氏族的客人！"]],["哦呵呵呵，那是当然！"],[function():void
            {
               ...
               {
                  FightManager.startFightWithWild(62);
               }]);
            }]);
         }
      }
