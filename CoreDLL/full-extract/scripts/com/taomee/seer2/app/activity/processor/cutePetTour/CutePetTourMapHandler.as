package com.taomee.seer2.app.activity.processor.cutePetTour
{
   ...
   public class CutePetTourMapHandler extends ActivityMapHandler
   {
      ...
      override public function process() : void
      {
         ...
         var _loc2_:uint = (_activityProcessor as CutePetTour).getCurrentPhaseIndex();
         if(Boolean(_loc2_) && SceneManager.active.mapID == _activityProcessor.getRelateMapIDArr()[0])
         {
            if(this._currentPhaseIndex != _loc2_)
            {
               this._currentPhaseIndex = _loc2_;
               _loc1_ = new Date(TimeManager.getServerTime() * 1000);
               this._isPhaseStarting = _loc1_.hours * 60 + _loc1_.minutes == (_activityProcessor as CutePetTour).getCurrentTimeVec()[2 * (this._currentPhaseIndex - 1)];
               this.createChildren();
            }
         }
      ...
      {
         QueueLoader.load(URLUtil.getActivityAnimation("3/fireBalloon"),"swf",this.onFireBalloonLoaded);
      }
      ...
      private function onFireBalloonLoaded(param1:ContentInfo) : void
      {
         ...
         this._fireBalloonMC.addEventListener("click",this.onFireBalloonClick);
         this._fireBalloonMC.addEventListener("enterFrame",this.onFireBalloonEnterFrame);
      ...
      private function removeFireBalloon() : void
      {
         ...
         {
            this._fireBalloonMC.removeEventListener("enterFrame",this.onFireBalloonEnterFrame);
