package com.taomee.seer2.app.controls.widget
{
   ...
   public class ChatInputFrameWidget extends Sprite implements IWidgetable
   {
      ...
      {
         this._msgInputTxt.addEventListener("focusIn",this.inFocusInput);
         this._msgInputTxt.addEventListener("focusOut",this.outFocusInput);
         this._sendMsgBtn.addEventListener("click",this.onMessageSend);
      }
      ...
      private function onMessageSend(param1:MouseEvent = null) : void
      {
         ...
         var _loc2_:String = StringUtil.trim(this._msgInputTxt.text);
         if(_loc2_ == "")
         {
            return;
         }
         ...
         {
            StatisticsManager.sendNovice("0x1003060A");
      ...
      {
         var _loc1_:String = StringUtil.trim(this._msgInputTxt.text);
