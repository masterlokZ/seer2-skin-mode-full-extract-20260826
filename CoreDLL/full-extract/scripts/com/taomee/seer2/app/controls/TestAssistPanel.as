package com.taomee.seer2.app.controls
{
   ...
   public class TestAssistPanel extends Sprite
   {
      ...
      private var _txtFilter:GlowFilter = new GlowFilter(0,1,2,2,10,1);
      ...
      private function createChildren() : void
      {
         ...
         this._mousePositionLabel = this.createTxt("dynamic",85,20);
         ...
         this._mousePositiontxt = this.createTxt("dynamic",80,20);
         this._mousePositiontxt.x = 83;
         this._mousePositiontxt.y = 24;
         addChild(this._mousePositiontxt);
         this._switchMapInput.addEventListener("keyDown",this.onEnterKeyDown);
         this._serverTimeTxt = this.createTxt("dynamic",180,20);
         this._serverTimeTxt.x = 2;
         this._serverTimeTxt.y = 44;
         addChild(this._serverTimeTxt);
         this._serverIdTxtLabel = this.createTxt("input",70,20,65280);
         this._serverIdTxtLabel.text = "协议号";
         this._serverIdTxtLabel.x = 2;
         this._serverIdTxtLabel.y = 86;
         addChild(this._serverIdTxtLabel);
         this._serverContentTxtLabel = this.createTxt("input",110,20,65280);
         ...
         this._serverBitLabelTxt = this.createTxt("input",180,40,65280);
         ...
         this._serverIdTxt = this.createTxt("input",40,20,65280);
         ...
         this._serverContentTxt = this.createTxt("input",250,20,65280);
         ...
         this._serverBitContentTxt = this.createTxt("input",250,20,65280);
         ...
         this._serverSim = this.createTxt("dynamic",60,20,65280);
         this._serverSim.x = 2;
         this._serverSim.y = 64;
         this._serverSim.mouseEnabled = true;
         this._serverSim.text = "协议测试";
         addChild(this._serverSim);
         this._serverSim.addEventListener("click",this.onShowServer);
         this._serverIdTxt.addEventListener("keyDown",this.onKeyDown);
         this._serverContentTxt.addEventListener("keyDown",this.onKeyDown);
         this._serverBitContentTxt.addEventListener("keyDown",this.onKeyDown);
      ...
      private function createBack() : Shape
      {
         ...
         _loc1_.beginFill(13421772,0.2);
         _loc1_.drawRect(0,0,150,44);
         _loc1_.endFill();
         return _loc2_;
      }
      ...
      private function createTxt(param1:String, param2:int, param3:int, param4:uint = 16777215) : TextField
      {
         ...
         _loc5_.filters = [this._txtFilter];
         return _loc5_;
      }
      ...
      private function onKeyDown(param1:KeyboardEvent) : void
      {
         ...
         {
            _loc3_ = this._serverContentTxt.text.split(",");
            _loc5_ = this._serverBitContentTxt.text.split(",");
            ...
            else if(_loc3_.length > 0 && _loc5_.length > 0)
            {
            ...
            if(this._serverContentTxt.text != "")
            {
               ...
               {
                  this.udpateByte(_loc4_,String(_loc3_[_loc2_]));
                  _loc2_++;
               }
      ...
      private function udpateByte(param1:LittleEndianByteArray, param2:String) : void
      {
         ...
         if(param2.charAt(0) == "(")
         {
            _loc5_ = param2.slice(1,param2.length - 1);
            _loc4_ = _loc5_.split("|");
