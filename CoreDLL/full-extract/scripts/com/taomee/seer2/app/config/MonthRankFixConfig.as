package com.taomee.seer2.app.config
{
   ...
   public class MonthRankFixConfig
   {
      ...
      {
         var _loc3_:MonthRankFixInfo = null;
         var _loc2_:Date = new Date(TimeManager.getPrecisionServerTime() * 1000);
         var _loc4_:String = _loc2_.fullYear + (String(_loc2_.month + 1 < 10 ? "0" + (_loc2_.month + 1) : _loc2_.month + 1));
         if(this._all.containsKey(_loc4_))
         {
            _loc3_ = this._all.getValue(_loc4_) as MonthRankFixInfo;
            if(param1 == 0)
            {
               return (this._all.getValue(_loc4_) as MonthRankFixInfo).list0;
            }
            if(param1 == 1)
            {
               return (this._all.getValue(_loc4_) as MonthRankFixInfo).list1;
            }
            if(param1 == 2)
            {
               return (this._all.getValue(_loc4_) as MonthRankFixInfo).list2;
            }
            if(param1 == 3)
            {
               return (this._all.getValue(_loc4_) as MonthRankFixInfo).list3;
            }
            if(param1 == 4)
            {
               return (this._all.getValue(_loc4_) as MonthRankFixInfo).list4;
            }
            if(param1 == 5)
            {
               return (this._all.getValue(_loc4_) as MonthRankFixInfo).list5;
            }
         }
         else
         {
      ...
      private function loadConfig() : void
      {
         if(!this._isLoaded && !this._isLoading)
         {
            ...
            QueueLoader.load(URLUtil.getActivityXML("MonthRankFix"),"text",this.onComplete);
         }
      }
      ...
      {
         var _loc2_:XML = null;
         var _loc1_:MonthRankFixInfo = null;
         this._allList0 = [];
         this._allList1 = [];
         this._allList2 = [];
         this._allList3 = [];
         ...
         {
            _loc1_ = new MonthRankFixInfo();
            _loc1_.date = String(_loc2_.@date);
            _loc1_.list0 = int(_loc2_.@list0);
            _loc1_.list1 = int(_loc2_.@list1);
            _loc1_.list2 = int(_loc2_.@list2);
            _loc1_.list3 = int(_loc2_.@list3);
            _loc1_.list4 = int(_loc2_.@list4);
            _loc1_.list5 = int(_loc2_.@list5);
            this._all.put(_loc1_.date,_loc1_);
            this._allList0.push(_loc1_.list0);
            this._allList1.push(_loc1_.list1);
            this._allList2.push(_loc1_.list2);
            this._allList3.push(_loc1_.list3);
            this._allList4.push(_loc1_.list4);
            this._allList5.push(_loc1_.list5);
         }
         this._isLoaded = true;
