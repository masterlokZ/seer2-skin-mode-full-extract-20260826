   ...
   public class WebJumpConfig
   {
      ...
      private function loadConfig() : void
      {
         if(!this._isLoaded && !this._isLoading)
         {
            ...
            QueueLoader.load(URLUtil.getActivityXML("WebJumpConfig"),"text",this.onComplete);
         }
      }
      ...
      {
         var _loc4_:XML = null;
         var _loc2_:JumpInfo = null;
         var _loc3_:XML = XML(param1.content);
         var _loc5_:XMLList = _loc3_.descendants("info");
         for each(_loc4_ in _loc5_)
         {
            _loc2_ = new JumpInfo();
            _loc2_.tid = uint(_loc4_.@tid);
            this._urlMap.put(uint(_loc4_.@id),_loc2_);
         }
         this._isLoaded = true;
