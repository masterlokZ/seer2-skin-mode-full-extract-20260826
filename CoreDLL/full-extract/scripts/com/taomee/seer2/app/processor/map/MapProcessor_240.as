package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_240 extends TitleMapProcessor
   {
      ...
      override public function init() : void
      {
         ...
         this.initStone();
         StatisticsManager.sendNovice("0x1003343F");
         if(QuestManager.isStepComplete(30051,3))
         {
            _loc1_ = MobileManager.getMobile(626,"npc");
            if(_loc1_)
            {
      ...
      private function initStone() : void
      {
         ...
         this._stone.addEventListener("mouseOver",this.onStoneMouseOver);
         this._stone.addEventListener("mouseOut",this.onStoneMouseOut);
         this._stone.addEventListener("click",this.onStoneClick);
      }
      ...
      {
         this._stone.removeEventListener("mouseOver",this.onStoneMouseOver);
         this._stone.removeEventListener("mouseOut",this.onStoneMouseOut);
         this._stone.removeEventListener("click",this.onStoneClick);
         this._stone = null;
         super.dispose();
      }
   }
