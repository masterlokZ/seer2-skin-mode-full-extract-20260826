package com.taomee.seer2.app.processor.activity.waitNewYearAct
{
   ...
   public class WaitNewYearAct
   {
      ...
      private function getURL(param1:Function) : void
      {
         ...
         QueueLoader.load(URLUtil.getActivityAnimation("waitNewYearAct/WaitNewYearAct"),"swf",function(param1:ContentInfo):void
         {
      ...
      {
         var _loc3_:int = 0;
         var _loc2_:int = this._curLeaveTime / 60;
         var _loc1_:Array = [0,0,0];
         while(_loc3_ < 3)
         {
            _loc1_[2 - _loc3_] = _loc2_ % 10;
            _loc2_ /= 10;
            _loc3_++;
         }
         ...
         while(_loc3_ < 3)
         {
            (this._actTip["time"]["num" + _loc3_] as MovieClip).gotoAndStop(_loc1_[_loc3_] + 1);
            _loc3_++;
         }
      }
      ...
      public function handleNpcHandle() : void
      {
         ...
         if(this._index != -1)
         {
            ...
            if(DateUtil.isInTime(new Date(2015,11,31,20,TIME_LIST[this._index][0],0),new Date(2015,11,31,20,TIME_LIST[this._index][1],59)))
            {
               ...
               if(!MobileManager.getMobile(NPC_ID_LIST[this._index],"npc"))
               {
      ...
      private function hideJustinNpc() : void
      {
         if(this._index == 11)
         {
            MobileManager.getMobile(NPC_ID_LIST[this._index],"npc").visible = false;
         }
      }
      ...
      {
         this._oldNpc = MobileManager.getMobile(NPC_ID_LIST[this._index],"npc");
