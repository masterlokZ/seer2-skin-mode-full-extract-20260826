package com.taomee.seer2.app.processor.quest.handler.branch.quest10092
{
   ...
   public class QuestMapHandler_10092_840 extends QuestMapHandler
   {
      ...
      override public function processMapComplete() : void
      {
         ...
         this.npc = MobileManager.getMobile(489,"npc");
         ...
         {
            NpcManager.addEventListener("actorArrive",this.onBattleDia);
         }
         if(QuestManager.isStepComplete(_quest.id,1))
         {
            NpcManager.addEventListener("actorArrive",this.onArriveNpc);
         }
         if(QuestManager.isAccepted(_quest.id) && SceneManager.prevSceneType == 2 && startFight == 1)
         {
            startFight = 0;
      ...
      private function againBattle() : void
      {
         ...
         {
            NpcManager.addEventListener("actorArrive",onBattleDia);
            NpcDialog.hide();
         }]);
      }
      ...
      private function onBattleDia(param1:NpcEvent) : void
      {
         ...
         {
            NpcManager.removeEventListener("actorArrive",this.onBattleDia);
      ...
      private function onArriveNpc(param1:NpcEvent) : void
      {
         ...
         if(param1.npcDefinition.id == 489)
         {
            ...
            _loc4_ = <node type="quest" name="奖品兑换" params="30013_1"/>;
            _loc3_ = <node type="quest" name="帮助抵抗袭击" params="30013_2"/>;
            param1.npcDefinition.resetFunctionalityData();
            param1.npcDefinition.addFunctionalityUnitAt(_loc2_,0);
            param1.npcDefinition.addFunctionalityUnitAt(_loc4_,1);
            param1.npcDefinition.addFunctionalityUnitAt(_loc3_,2);
            DialogPanel.addEventListener("customUnitClick",this.onUnit);
         }
      }
      ...
      private function onUnit(param1:DialogPanelEvent) : void
      {
         ...
         {
            ServerBufferManager.getServerBuffer(18,this.getBuffer);
         }
      }
      ...
      private function initShoot() : void
      {
         ...
         ShootController.addEventListener("playEnd",this.chekTarget);
         (SceneManager.active as LobbyScene).hideToolbar();
         this.timer = new Timer(200);
         this.timer.addEventListener("timer",this.checkHitNpc);
         this.timer.start();
         this.createFireBall();
      }
      ...
      private function overInteractive() : void
      {
         ...
         ++this.interactiveNum;
         ShootController.removeEventListener("playEnd",this.chekTarget);
         ServerBufferManager.updateServerBuffer(18,0,this.interactiveNum);
         this.timer.stop();
         this.timer.reset();
      ...
      private function createFireBall() : void
      {
         ...
         var _loc1_:int = this.bulletNum % 3;
         if(_loc1_ == 0)
         {
            _loc2_ = _processor.resLib.getMovieClip("fireball_1");
         }
         else if(_loc1_ == 1)
         {
            _loc2_ = _processor.resLib.getMovieClip("fireball_2");
         }
         else
         {
            _loc2_ = _processor.resLib.getMovieClip("fireball_3");
         }
         ++this.bulletNum;
      ...
      {
         NpcManager.removeEventListener("actorArrive",this.onActorArriveShow);
         NpcManager.removeEventListener("actorArrive",this.onArriveNpc);
         QuestManager.removeEventListener("stepComplete",this.onCompleteStep1);
         ...
         if(this.timer)
         {
            this.timer.stop();
            this.timer.reset();
            this.timer.removeEventListener("timer",this.checkHitNpc);
         }
