package com.taomee.seer2.app.processor.quest.handler.branch.quest10135
{
   ...
   public class Game10135
   {
      ...
      private function initMC0() : void
      {
         ...
         var _loc2_:Rectangle = new Rectangle(-230,-150,500,300);
         this._pi.startDrag(true,_loc2_);
         this._pi.mouseChildren = false;
         this._pi.mouseEnabled = false;
         this._long.addEventListener("mouseDown",this.onDeleteHandler);
         this._okBtn.addEventListener("mouseDown",this.onOkHandler);
         var _loc1_:int = 0;
         while(_loc1_ < 5)
         {
      ...
      private function onDeleteHandler(param1:MouseEvent) : void
      {
         ...
         this._pi.addEventListener("enterFrame",this.enterFrameHandler);
      ...
      private function enterFrameHandler(param1:Event) : void
      {
         ...
         this._long.addEventListener("mouseUp",this.onDeleteOverHandler);
      }
      
      private function onDeleteOverHandler(param1:MouseEvent) : void
      {
         this._pi.removeEventListener("enterFrame",this.enterFrameHandler);
      }
      ...
      {
         this._pi.removeEventListener("enterFrame",this.enterFrameHandler);
         this._long.removeEventListener("mouseDown",this.onDeleteHandler);
         this._long.removeEventListener("mouseUp",this.onDeleteOverHandler);
         this._okBtn.removeEventListener("mouseDown",this.onOkHandler);
      ...
      private function initMC1() : void
      {
         ...
         while(_loc2_ < 5)
         {
            _loc1_ = this._currMC["d" + _loc2_];
            this._coinsList.push(_loc1_);
            _loc2_++;
         }
