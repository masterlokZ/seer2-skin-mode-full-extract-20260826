package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80109 extends TitleMapProcessor
   {
      ...
      private function kengTeActInit() : void
      {
         ...
         {
            if(FightManager.currentFightRecord.initData.positionIndex == 774)
            {
               DayLimitListManager.getDaylimitList(DAY_LIST,function(param1:DayLimitListInfo):void
               {
                  ...
                  ActiveCountManager.requestActiveCount(FOR_LIST[0],function(param1:uint, param2:int):void
                  {
                     ...
                     if(FOR_LIST[0] == type)
                     {
                        ...
                        ModuleManager.addEventListener("KengTeFightResultPanel","dispose",(function():*
                        {
                           var onDispose:Function;
                           return onDispose = function(param1:ModuleEvent):void
                           {
                              ModuleManager.removeEventListener("KengTeFightResultPanel","dispose",onDispose);
                              if(leaveNum > 0)
                              {
                                 createNpc();
                              }
      ...
      private function initHurtAct() : void
      {
         ...
         this._func.addEventListener("click",this.onFunc);
         this._miBuy = _map.front["miBuy"];
         this._miBuy.addEventListener("click",this.onMiBuy);
      ...
      private function updateHurtAct() : void
      {
         ...
         {
            _scoreVal = param2;
            _gameNum.text = param2.toString() + "/" + (500).toString();
            _miBuy.mouseEnabled = true;
            var _loc3_:Number = param2 / 500;
            if(_loc3_ > 1)
            {
               _loc3_ = 1;
            }
      ...
      private function kengTeActInitDispose() : void
      {
         ...
         if(this._func)
         {
            this._func.removeEventListener("click",this.onFunc);
         }
         this._hurtNum = null;
         this._gameNum = null;
         this._bar = null;
      ...
      private function onNpcClick(param1:MouseEvent) : void
      {
         ...
         {
            FightManager.startFightWithWild(774);
         }]);
      }
