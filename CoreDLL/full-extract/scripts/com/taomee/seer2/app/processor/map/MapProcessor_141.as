package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_141 extends TitleMapProcessor
   {
      ...
      override public function init() : void
      {
         this.initFlower();
         this.initGame();
         StatisticsManager.sendNovice("0x10033418");
      }
      
      private function initFlower() : void
      {
         this._lightFlowerRightRewardMC = _map.content["lightFlowerRight"] as MovieClip;
         this._lightFlowerRightRewardMC.addEventListener("lightFlowerRight1",this.onFlowerReward);
      }
      
      private function onFlowerReward(param1:Event) : void
      {
         this._lightFlowerRightRewardMC.removeEventListener("lightFlowerRight1",this.onFlowerReward);
      ...
      private function initGame() : void
      {
         ...
         this._gameTrigger.addEventListener("click",this.onGameStart);
         this._gameTrigger.addEventListener("rollOver",this.onTriggerOver);
         this._gameTrigger.addEventListener("rollOut",this.onTriggerOut);
      }
      
      private function onGameStart(param1:MouseEvent) : void
      {
         StatisticsManager.sendNovice("0x10033562");
         ModuleManager.toggleModule(URLUtil.getAppModule("MiniGameIntroPanel"),"正在打开小游戏...",{"gameId":5});
      }
      ...
      override public function dispose() : void
      {
         ...
         {
            this._gameTrigger.removeEventListener("click",this.onGameStart);
            this._gameTrigger.removeEventListener("rollOver",this.onTriggerOver);
            this._gameTrigger.removeEventListener("rollOut",this.onTriggerOut);
