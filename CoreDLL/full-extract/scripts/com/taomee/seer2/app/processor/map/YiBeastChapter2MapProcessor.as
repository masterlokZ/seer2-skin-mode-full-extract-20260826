   ...
   public class YiBeastChapter2MapProcessor extends MapProcessor
   {
      ...
      private function getCount(param1:uint, param2:uint) : void
      {
         ...
         if(type == this.LIMIT_ID_LIST[this.currentIndex])
         {
            ...
            {
               if(SceneManager.prevSceneType == 2 && FightManager.currentFightRecord.initData.positionIndex == this.FIGHT_ID_LIST[this.currentIndex])
               {
      ...
      private function createMonster() : void
      {
         ...
         while(_loc1_ < 3)
         {
            _loc2_ = new SpawnedPet(this.MONSTER_ID_LIST[this.currentIndex],this.FIGHT_ID_LIST[this.currentIndex],0);
