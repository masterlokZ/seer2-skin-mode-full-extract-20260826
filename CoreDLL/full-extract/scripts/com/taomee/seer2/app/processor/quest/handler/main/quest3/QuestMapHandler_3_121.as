package com.taomee.seer2.app.processor.quest.handler.main.quest3
{
   ...
   public class QuestMapHandler_3_121 extends QuestMapHandler
   {
      ...
      {
         if(SceneManager.prevSceneType == 2 && FightManager.fightWinnerSide == 1 && FightManager.endReason == 7)
         {
      ...
      {
         var _loc1_:int = 0;
         if(SceneManager.prevSceneType == 2 && FightManager.fightWinnerSide == 1 && FightManager.endReason == 3)
         {
            _loc1_ = _quest.getStepData(5,0);
            _loc1_++;
            _quest.setStepData(5,0,_loc1_);
            if(_loc1_ >= 1)
            {
               QuestManager.addEventListener("stepComplete",this.onCompleteStep5);
               QuestManager.completeStep(_quest.id,5);
            }
            else
            {
               QuestManager.addEventListener("stepUpdateBuffer",this.onUpdateStepBuffer);
               QuestManager.setStepBufferServer(_quest.id,5);
            }
         }
         else
         {
      ...
      private function onUpdateStepBuffer(param1:QuestEvent) : void
      {
         ...
         if(param1.questId == _quest.id)
         {
            ...
            _loc2_ = 1 - _quest.getStepData(5,0);
            if(_loc2_ > 0)
            {
      ...
      private function checkFightFlag() : void
      {
         ...
         {
            _loc1_ = MobileManager.getMobileVec("spawnedPet")[0] as SpawnedPet;
