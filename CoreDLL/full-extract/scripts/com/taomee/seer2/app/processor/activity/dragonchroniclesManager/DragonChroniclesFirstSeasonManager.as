package com.taomee.seer2.app.processor.activity.dragonchroniclesManager
{
   ...
   public class DragonChroniclesFirstSeasonManager
   {
      ...
      private static var petVector:Array = [];
      ...
      private static var petTwoVector:Array = [];
      ...
      public static function init(param1:int = -1) : void
      {
         ...
         QueueLoader.load(URLUtil.getActivityAnimation("dragonchronicles/dragonchroniclesfirst"),"swf",onResLoaded);
      }
      ...
      private static function createFightPet() : void
      {
         ...
         if(winCount < 9)
         {
            ...
            while(_loc2_ < MAXNUM)
            {
               _loc1_ = generateRandom();
               if(_loc1_ != -1)
               {
                  _loc3_ = new SpawnedPet(531,fightOneID[_loc1_],levelList[_loc1_]);
      ...
      private static function createFightTwoPet() : void
      {
         ...
         while(_loc2_ < 9)
         {
            if(BitUtil.getBit(winTwoBit,_loc2_) == false)
            {
               _loc1_ = new Mobile();
               _loc1_.setPostion(new Point(petPos[_loc2_][0],petPos[_loc2_][1]));
               _loc1_.label = petName[_loc2_];
               _loc1_.resourceUrl = URLUtil.getNpcSwf(petTwoIDArr[_loc2_]);
               _loc1_.buttonMode = true;
               _loc1_.addEventListener("click",onFight);
      ...
      private static function onOperationThree() : void
      {
         ...
         _longMc.addEventListener("click",onBuyLong);
         _harmMc = _resMc["harmMc"];
         _itemShop = _resMc["itemShop"];
         _itemShop.addEventListener("click",onItemShop);
         createBlackDragon();
         updateShow();
      }
      ...
      private static function createBlackDragon() : void
      {
         ...
         _npcBlackDragon.addEventListener("click",onFightBlackDragon);
         MobileManager.addMobile(_npcBlackDragon,"npc");
      }
      ...
      public static function dispose() : void
      {
         ...
         {
            MobileManager.removeMobile(petVector[_loc1_],"spawnedPet");
            _loc1_++;
         }
         _loc1_ = 0;
         while(_loc1_ < petTwoVector.length)
         {
            MobileManager.removeMobile(petTwoVector[_loc1_].obj,"npc");
            _loc1_++;
         }
         if(_npcBlackDragon != null)
         {
            MobileManager.removeMobile(_npcBlackDragon,"npc");
         }
         if(_resMc != null)
         {
