package com.taomee.seer2.app.processor.quest.handler.main.quest9
{
   ...
   public class QuestMapHandler_9_213 extends QuestMapHandler
   {
      ...
      private function processStep5() : void
      {
         ...
         this._dragTimer.addEventListener("timer",this.onTimer);
         MovieClipUtil.getSwfContent(URLUtil.getQuestAnimation("9/cook"),function(param1:MovieClip):void
         {
            ...
            MovieClipUtil.getChildList(mc,2,["bottle","water","chilli","pepper","salt","spoon","hand","xuanzhuan","hotArea","areaHolder"],function(param1:Vector.<DisplayObject>):void
            {
               ...
               while(_loc5_ < 6)
               {
                  _loc2_ = param1[_loc5_] as MovieClip;
                  _loc2_.gotoAndStop(1);
                  _loc2_.visible = false;
                  _dragMcVec.push(_loc2_);
                  _dragPosVec.push(new Point(_loc2_.x,_loc2_.y));
                  _loc5_++;
               }
               _dragIndex = 0;
               updateDragMc();
               ...
               _hotArea.addEventListener("mouseOut",onHotAreaOut);
               _hotArea.addEventListener("mouseDown",onHotAreaDown);
               _hotArea.addEventListener("mouseUp",onHotAreaUp);
               _hotArea.addEventListener("mouseMove",onHotAreaMove);
               _hotArea.addEventListener("click",onHotAreaClick);
               DisplayObjectUtil.disableSprite(_hotArea);
               _loc4_ = param1[9] as MovieClip;
               ...
               while(_loc5_ < 10)
               {
                  _matchAreaArr.push(DisplayObject(_loc4_["area" + _loc5_]).getBounds(LayerManager.stage));
                  _loc5_++;
               }
            });
         });
      ...
      private function updateDragMc() : void
      {
         ...
         this._dragMc.addEventListener("rollOver",this.onDragMcOver);
         this._dragMc.addEventListener("rollOut",this.onDragMcOut);
         this._dragMc.addEventListener("click",this.onDragMcClick);
      ...
      private function onDragMcClick(param1:MouseEvent) : void
      {
         ...
         this._dragMc.removeEventListener("rollOver",this.onDragMcOver);
         this._dragMc.removeEventListener("rollOut",this.onDragMcOut);
         this._dragMc.removeEventListener("click",this.onDragMcClick);
      ...
      private function closeCook(param1:Boolean) : void
      {
         ...
         this._dragTimer.removeEventListener("timer",this.onTimer);
         this._dragTimer.stop();
         if(this._hotArea != null)
         {
            this._hotArea.removeEventListener("mouseOut",this.onHotAreaOut);
            this._hotArea.removeEventListener("mouseDown",this.onHotAreaDown);
            this._hotArea.removeEventListener("mouseUp",this.onHotAreaUp);
            this._hotArea.removeEventListener("mouseMove",this.onHotAreaMove);
            this._hotArea.removeEventListener("click",this.onHotAreaClick);
         }
