package com.taomee.seer2.app.processor.quest.handler.activity.quest30034
{
   ...
   public class Game30034
   {
      ...
      public function Game30034(param1:Function)
      {
         ...
         QueueLoader.load(URLUtil.getActivityAnimation("xegg/game4"),"swf",this.onResLoaded);
      }
      ...
      private function initEventListener() : void
      {
         ...
         this._recoverTimer.addEventListener("timer",this.timerHandler);
         this._recoverTimer.start();
         LayerManager.topLayer.addEventListener("click",this.mouseClickHandler);
         this._clockTimer = new Timer(600);
         this._clockTimer.addEventListener("timer",this.clockTick);
      ...
      private function clockTick(param1:TimerEvent) : void
      {
         ...
         if(this._ui["timerClock"].currentFrame >= this._ui["timerClock"].totalFrames)
         {
            ...
            this._clockTimer.removeEventListener("timer",this.clockTick);
      ...
      private function mouseClickHandler(param1:MouseEvent) : void
      {
         ...
         var _loc2_:uint = this._hurtClickCount / 2;
      ...
      public function dispose() : void
      {
         this._clockTimer.reset();
         this._recoverTimer.reset();
         this._recoverTimer.removeEventListener("timer",this.timerHandler);
         LayerManager.topLayer.removeEventListener("click",this.mouseClickHandler);
         this._clockTimer.removeEventListener("timer",this.clockTick);
