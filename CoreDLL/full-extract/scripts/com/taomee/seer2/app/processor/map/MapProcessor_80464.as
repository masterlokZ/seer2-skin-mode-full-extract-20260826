package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_80464 extends MapProcessor
   {
      ...
      protected function reqResult(param1:MouseEvent) : void
      {
         ...
         this.ghost.stop();
         SwapManager.swapItem(3977,1,function(param1:IDataInput):void
         {
            var data:IDataInput = param1;
            new SwapInfo(data);
            ActiveCountManager.requestActiveCount(205794,function(param1:int, param2:int):void
            {
               var type:int = param1;
               ...
               {
                  stopMove();
                  ModuleManager.addEventListener("BlackShootingResultPanel","hide",startMove);
                  ModuleManager.toggleModule(URLUtil.getAppModule("BlackShootingResultPanel"),"",count);
               }
               else
               {
                  DayLimitManager.getDoCount(1700,function(param1:int):void
                  {
                     if(param1 >= 10)
                     {
      ...
      {
         ModuleManager.removeEventListener("BlackShootingResultPanel","hide",this.startMove);
         this.initMouse();
      }
