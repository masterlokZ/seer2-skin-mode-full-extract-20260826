package com.taomee.seer2.app.processor.quest.handler.branch.quest10125
{
   ...
   public class QuestMapHandler_10125_Mine extends QuestMapHandler
   {
      ...
      private var yData:XML = <mineport id="2" resId="2" name="黄色矿脉" pos="820,270" actorPos="800,270" limit="3" aniPos="0,0"
                               serverConfigId="2"/>;
      
      private var bData:XML = <mineport id="3" resId="1" name="蓝色矿脉" pos="550,220" actorPos="398,400" limit="3"
                               serverConfigId="4"/>;
      
      private var rData:XML = <mineport id="2" resId="3" name="红色矿脉" pos="710,415" actorPos="135,260" limit="3"
                               serverConfigId="1"/>;
      ...
      override public function processMapComplete() : void
      {
         ...
         {
            ServerBufferManager.getServerBuffer(46,this.getBuffer);
      ...
      {
         var _loc2_:XML = null;
         this.yellowNum = param1.readDataAtPostion(9);
         this.blueNum = param1.readDataAtPostion(10);
         this.redNum = param1.readDataAtPostion(11);
      ...
      private function alertTo220() : void
      {
         if(this.yellowNum + this.blueNum + this.redNum > 5)
         {
            ...
            {
               QuestManager.addEventListener("stepComplete",onSetp);
      ...
      private function onSetp(param1:QuestEvent) : void
      {
         ...
         {
            QuestManager.removeEventListener("stepComplete",this.onSetp);
      ...
      private function createdMinePort(param1:XML) : void
      {
         ...
         this.minePort.addEventListener("click",this.toMine,false,1000);
      }
      ...
      override public function processMapDispose() : void
      {
         ...
         if(this.minePort)
         {
            this.minePort.removeEventListener("click",this.toMine);
         }
      ...
      private function toMine(param1:MouseEvent) : void
      {
         ...
         MovieClipUtil.playMc(_mineAnimation,1,_mineAnimation.totalFrames,function():void
         {
            ...
            {
               ++yellowNum;
               ServerBufferManager.updateServerBuffer(46,9,yellowNum);
               num = yellowNum;
            }
            if(mineId == 400004)
            {
               ++blueNum;
               ServerBufferManager.updateServerBuffer(46,10,blueNum);
               num = blueNum;
            }
            if(mineId == 400002)
            {
               ++redNum;
               ServerBufferManager.updateServerBuffer(46,11,redNum);
               num = redNum;
            }
            if(num > 1)
            {
