package com.taomee.seer2.app.processor.quest.handler.main.quest11.interactive
{
   ...
   public class BadPeople
   {
      ...
      public function BadPeople(param1:MovieClip, param2:BloodBar, param3:int, param4:int, param5:int, param6:BloodBar, param7:ResourceLibrary)
      {
         ...
         this._bloodBar.addEventListener("bloodOver",this.onBloodOver);
      }
      ...
      private function onTimer(param1:TimerEvent) : void
      {
         ...
         if(this.isAvalible())
         {
            _loc4_ = new Point(_loc2_.pos.x,_loc2_.pos.y - 35);
            _loc3_ = new Point(this._displayMC.x + 221,this._displayMC.y + 313);
            ShootController.execute(0,_loc2_.id,_loc2_,_loc4_,_loc3_);
         }
      }
      
      private function isAvalible() : Boolean
      {
         var _loc2_:Point = ActorManager.getActor().pos;
         var _loc1_:int = _loc2_.x - this._x;
         var _loc3_:int = _loc2_.y - this._y;
         if(Math.sqrt(_loc1_ * _loc1_ + _loc3_ * _loc3_) < 200)
         {
            return true;
         }
         return false;
