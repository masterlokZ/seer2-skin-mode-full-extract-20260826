package com.taomee.seer2.app.processor.activity.bestKingEndAct
{
   ...
   public class BestKingEndAct
   {
      ...
      private function init() : void
      {
         ...
         (this._mapModel.content["award"] as SimpleButton).addEventListener("click",this.onGetAward);
         (this._mapModel.content["winnerMc"] as MovieClip).buttonMode = true;
         (this._mapModel.content["winnerMc"] as MovieClip).addEventListener("click",this.onWinner);
         date = new Date(TimeManager.getServerTime() * 1000);
         ServerBufferManager.getServerBuffer(402,function(param1:ServerBuffer):void
         {
            ...
            if(server.readDataAtPostion(date.date) == 0)
            {
               ...
               {
                  ServerBufferManager.updateServerBuffer(402,date.date,1);
                  update();
               },true,true,2,true);
            }
            else
      ...
      private function onGetAward(param1:MouseEvent) : void
      {
         ...
         SwapManager.swapItem(4166,1,function(param1:IDataInput):void
         {
            new SwapInfo(param1);
            (_mapModel.content["award"] as SimpleButton).visible = false;
      ...
      {
         (this._mapModel.content["award"] as SimpleButton).removeEventListener("click",this.onGetAward);
         (this._mapModel.content["winnerMc"] as MovieClip).removeEventListener("click",this.onWinner);
         SeatTipsManager.removeSeat(SEAT_POS,3853);
      }
   }
}
