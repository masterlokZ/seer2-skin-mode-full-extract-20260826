package com.taomee.seer2.app.processor.quest.handler.story.quest18
{
   ...
   public class QuestMapHandler_18_540 extends QuestMapHandler
   {
      ...
      override public function processMapComplete() : void
      {
         ...
         if(SceneManager.prevSceneType == 2 && QuestManager.isStepComplete(_quest.id,5) == false && _flghtIndex == 1)
         {
            this._count = 0;
            this.initQuest4(102);
            if(FightManager.fightWinnerSide == 1 && _flghtIndex == 1)
            {
               _flghtIndex = 0;
               this.win();
            }
         ...
         MovieClipUtil.playFullScreen(URLUtil.getQuestFullScreenAnimation("18_0"),function():void
         {
            ...
            NpcDialog.show(11,"多罗",[[2,"好冷啊~~~队长！虽然我是一个机器人，看到这样的景象还是会觉得……好冷！！"]],["……这就是冰沁氏族？"],[function():void
            {
               NpcDialog.show(11,"多罗",[[2,"这里一个人……一个精灵……一个赛尔都没有！我们该怎么办？？"]],["四处检查一下！！"],[function():void
               {
                  ...
                  {
                     QuestManager.addEventListener("stepComplete",onQuestStep);
      ...
      {
         QuestManager.removeEventListener("stepComplete",this.onQuestStep);
         if(QuestManager.isStepComplete(_quest.id,3) && QuestManager.isStepComplete(_quest.id,4) == false)
         {
            if(this._mc_1)
            {
               ...
               this._mc_1.addEventListener("click",this.onClick);
            }
            else
            {
               this.initNpc();
            ...
            if(info.id == 3)
            {
               ...
               if(this._count == 2)
               {
                  MovieClipUtil.playMc(this._mc_2,76,this._mc_2.totalFrames,function():void
                  {
                     ...
                     _mc_2.addEventListener("click",onMC2Click);
                  },true);
               }
            }
         }
      ...
      private function startFight() : void
      {
         if(PetInfoManager.getFirstPetInfo().hp <= 1)
         {
            ...
            _processor.showMouseHintAt(492,221);
            this._mc_2.addEventListener("click",this.onMC2Click);
            return;
         }
         _flghtIndex = 1;
      ...
      {
         this._mc_2.removeEventListener("click",this.onMC2Click);
         this.startFight();
      }
      ...
      private function onClick(param1:MouseEvent) : void
      {
         ...
         NpcDialog.show(10,"巴蒂",[[0,"嗯……我什么都没有找到……不过！伊娃博士不是给了我们一个生命勘测仪吗？这个时候应该可以派上用处了。"]],["（使用生命勘测仪）"],[function():void
         {
            ...
            _mc_1.removeEventListener("click",onClick);
      ...
      private function noWin() : void
      {
         this._mc_2.buttonMode = true;
         _processor.showMouseHintAt(492,221);
         this._mc_2.addEventListener("click",function(param1:MouseEvent):void
         {
            var event:MouseEvent = param1;
            _mc_2.removeEventListener("click",arguments.callee);
      ...
      override public function processMapDispose() : void
      {
         ...
         if(this._mc_1)
         {
            this._mc_1.removeEventListener("click",this.onClick);
         }
