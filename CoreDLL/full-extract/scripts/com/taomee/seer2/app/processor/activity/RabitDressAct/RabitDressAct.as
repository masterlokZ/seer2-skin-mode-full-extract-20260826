package com.taomee.seer2.app.processor.activity.RabitDressAct
{
   ...
   public class RabitDressAct
   {
      ...
      private function getURL() : void
      {
         ...
         QueueLoader.load(URLUtil.getActivityAnimation("rabitDressAct"),"swf",function(param1:ContentInfo):void
         {
      ...
      private function init() : void
      {
         ...
         if(RabitDressAct.state[0] == 0)
         {
            ...
            {
               var itemHandle2:Function;
               return itemHandle2 = function(param1:ItemEvent):void
               {
                  var evt:ItemEvent = param1;
                  ItemManager.removeEventListener1("requestSpecialItemSuccess",itemHandle2);
                  if(ItemManager.getSpecialItem(603064) == null)
                  {
                     state[2] = 0;
                  }
                  else
                  {
                     state[2] = ItemManager.getSpecialItem(603064).quantity;
                  }
                  ServerBufferManager.getServerBuffer(100,function(param1:ServerBuffer):void
                  {
                     ...
                     if(!_isPlay)
                     {
                        ...
                        MovieClipUtil.playMc(_scenMc,2,_scenMc.totalFrames,function():void
                        {
                           ...
                           _scenMc = null;
                           ServerBufferManager.updateServerBuffer(100,1,1);
                           state[0] = 1;
                           if(state[1] == 0)
                           {
                              state[1] = 386;
      ...
      private function addMonster() : void
      {
         ...
         while(_loc2_ < 5)
         {
            _loc1_ = new SpawnedPet(170,424,0);
      ...
      private function addMonsterClick() : void
      {
         ...
         var _loc2_:Vector.<Mobile> = MobileManager.getMobileVec("spawnedPet");
         for each(_loc1_ in _loc2_)
         {
            _loc1_.addEventListener("click",this.onMonsterClick);
         }
      }
      ...
      private function removeMonsterClick() : void
      {
         ...
         for each(_loc1_ in _loc2_)
         {
            _loc1_.removeEventListener("click",this.onMonsterClick);
         }
      }
      ...
      private function askDress() : void
      {
         if(state[2] >= 25)
         {
            SwapManager.swapItem(1252,1,function(param1:IDataInput):void
            {
               new SwapInfo(param1);
               state[3] = 1;
               dressTwoHandle();
      ...
      {
         ItemManager.addEventListener1("requestSpecialItemSuccess",this.getItemHandle1);
      ...
      private function getItemHandle1(param1:ItemEvent) : void
      {
         ...
         var evt:ItemEvent = param1;
         ItemManager.removeEventListener1("requestSpecialItemSuccess",this.getItemHandle1);
         if(ItemManager.getSpecialItem(603064) == null)
         {
            tpNum = 0;
         }
         ...
         {
            tpNum = int(ItemManager.getSpecialItem(603064).quantity);
         }
         if(tpNum > state[2])
         {
            state[2] = tpNum;
      ...
      private function onSwitch(param1:SceneEvent) : void
      {
         ...
         if(SceneManager.prevMapID == 80020 && SceneManager.active.type != 2)
         {
