package com.taomee.seer2.app.processor.quest.handler.activity.quest30003
{
   ...
   public class QuestMapHandler_30003_124 extends QuestMapHandler
   {
      ...
      override public function processMapComplete() : void
      {
         ...
         if(SceneManager.prevSceneType == 2 && QuestManager.isStepComplete(_quest.id,1) && QuestManager.isStepComplete(_quest.id,2) == false && this._fightStatus)
         {
         ...
         {
            _loc1_ = _quest.getStepData(_quest.getCurrentOrNextStep().id,0);
            if(_loc1_ == 0)
            {
               this.loadClothes();
            }
      ...
      private function loadNpc() : void
      {
         ...
         this._mc_1.addEventListener("click",this.onFightHandler);
      }
      ...
      {
         this._mc_1.removeEventListener("click",this.onFightHandler);
         NpcDialog.show(108,"黑衣人",[[0,"搞……什么……“伦家”都穿成这样了！还被发现了！哦妈咪妈咪哄！快看“灰机”！！！！"]],["切！你让我看我就看？"],[function():void
         {
            ...
            MovieClipUtil.playMc(_mc_0,1,_mc_0.totalFrames,function():void
            {
               NpcDialog.show(108,"黑衣人",[[0,"坑爹啊！我的衣服啊！俺娘新给俺织的！！！我们走着瞧！！！"]],[" 萨伦帝国的爪牙！！！"],[function():void
               {
                  ...
                  {
                     QuestManager.addEventListener("stepComplete",onStep2Handler);
      ...
      private function loadClothes() : void
      {
         ...
         this._mc_2.addEventListener("click",this.onPickUpCloth);
      }
      
      private function onPickUpCloth(param1:MouseEvent) : void
      {
         this._mc_2.removeEventListener("click",this.onPickUpCloth);
      ...
      private function prepareLoadPanel() : void
      {
         ...
         this._mc_0.addEventListener("click",this.onLoadPanel);
      }
      ...
      private function loadPanel() : void
      {
         ...
         this._choosePanel.addEventListener("select",this.onContinue);
      }
      
      private function onContinue(param1:Event) : void
      {
         var e:Event = param1;
         this._choosePanel.removeEventListener("select",this.onContinue);
