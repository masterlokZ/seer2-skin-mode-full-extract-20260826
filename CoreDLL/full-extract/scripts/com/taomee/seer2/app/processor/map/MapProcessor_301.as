package com.taomee.seer2.app.processor.map
{
   ...
   public class MapProcessor_301 extends TitleMapProcessor
   {
      ...
      override public function init() : void
      {
         this.initLightVec();
         this.initGame();
         StatisticsManager.sendNovice("0x1003343C");
      ...
      private function initLightVec() : void
      {
         ...
         var _loc2_:MovieClip = _map.content["light1"];
         var _loc1_:MovieClip = _map.content["light2"];
      ...
      private function initGame() : void
      {
         ...
         this._gameBtn.addEventListener("click",this.onGame);
      }
      
      private function onGame(param1:MouseEvent) : void
      {
         StatisticsManager.sendNovice("0x10033569");
         ModuleManager.toggleModule(URLUtil.getAppModule("MiniGameIntroPanel"),"正在打开小游戏...",{"gameId":6});
      }
      ...
      private function initLightInteracter() : void
      {
         ...
         while(_loc1_ < this._lightVec.length)
         {
            ...
            this._lightVec[_loc1_].addEventListener("mouseOver",this.onLightOver);
            this._lightVec[_loc1_].addEventListener("mouseOut",this.onLightOut);
            _loc1_++;
         }
         this._lightVec[1].addEventListener("click",this.onLightClick);
      }
      ...
      private function closeLightInteracter() : void
      {
         ...
         while(_loc1_ < this._lightVec.length)
         {
            this._lightVec[_loc1_].removeEventListener("mouseOver",this.onLightOver);
            this._lightVec[_loc1_].removeEventListener("mouseOut",this.onLightOut);
      ...
      private function clearGame() : void
      {
         if(this._gameBtn)
         {
            this._gameBtn.removeEventListener("click",this.onGame);
            this._gameBtn = null;
         }
      }
