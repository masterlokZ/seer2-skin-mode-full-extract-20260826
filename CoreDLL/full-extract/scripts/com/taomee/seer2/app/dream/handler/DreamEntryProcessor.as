package com.taomee.seer2.app.dream.handler
{
   ...
   public class DreamEntryProcessor
   {
      ...
      private function initActorCloseOpenEye() : void
      {
         ...
         {
            QueueLoader.load(URLUtil.getDreamSceneAnimation(),"swf",this.onCloseOpenEyeLoadedComplete);
         }
         else
         {
      ...
      private function resetCloseOpenEye() : void
      {
         ...
         if(DreamManager.currentDreamNpcID > 0)
         {
            ...
            this._openEye.addEventListener("enterFrame",this.onOpenedEye);
            this._openEye.play();
         }
         DreamManager.addEventListener("enterDream",this.onActorEnterDream);
         this.processNpc();
      }
      ...
      private function onOpenedEye(param1:Event) : void
      {
         ...
         {
            this._openEye.removeEventListener("enterFrame",this.onOpenedEye);
      ...
      {
         DreamManager.removeEventListener("enterDream",this.onActorEnterDream);
         LayerManager.topLayer.addChild(this._closeEye);
         this._closeEye.addEventListener("enterFrame",this.onClosedEye);
         this._closeEye.play();
      }
      ...
      private function onClosedEye(param1:Event) : void
      {
         ...
         {
            this._closeEye.removeEventListener("enterFrame",this.onClosedEye);
      ...
      private function enterDreamMap() : void
      {
         ...
         SceneManager.changeScene(1,_loc1_);
      }
      ...
      private function clearNpc() : void
      {
         ...
         {
            _loc2_ = this._dreamNpcVec.length;
            _loc1_ = 0;
            while(_loc1_ < _loc2_)
            {
               MobileManager.removeMobile(this._dreamNpcVec[_loc1_],"npc");
               _loc1_++;
            }
         }
      ...
      private function setDreamNpcAction() : void
      {
         ...
         var _loc1_:int = Math.floor(Math.random() * this._dreamNpcVec.length);
         var _loc3_:uint = 0;
