package com.taomee.seer2.app.arena.resource
{
   ...
   public class ResourceQueue extends EventDispatcher
   {
      ...
      public function startLoad() : void
      {
         ...
         this._isStart = true;
         dispatchEvent(new ResourceQueueEvent("queueStart"));
      ...
      private function startLoadResource() : void
      {
         ...
         {
            _loc1_.addEventListener("progress",this.onResourceProgress);
            _loc1_.addEventListener("itemComplete",this.onResourceLoaded);
      ...
      {
         dispatchEvent(new ResourceQueueEvent("queueError"));
         ++this._loadedCount;
      ...
      private function onResourceLoaded(param1:ResourceQueueEvent) : void
      {
         ...
         _loc2_.removeEventListener("progress",this.onResourceProgress);
         _loc2_.removeEventListener("itemComplete",this.onResourceLoaded);
         ++this._loadedCount;
         this._mainLoadedPercent = this._loadedCount / this._totalToLoad;
         this.checkQueueComplete();
      }
      ...
      private function checkQueueComplete() : void
      {
         ...
         {
            dispatchEvent(new ResourceQueueEvent("queueComplete"));
         }
      }
      ...
      public function cancelAll() : void
      {
         ...
         var _loc1_:ResourceDefinition = null;
         for each(_loc2_ in this._itemVec)
         {
            QueueLoader.cancel(_loc2_.url,_loc2_.completeHandler);
         }
         for each(_loc1_ in this._cacheVec)
         {
            CacheManager.cancel(_loc1_.url,_loc1_.type,_loc1_.completeHandler);
         }
         this._itemVec = new Vector.<ResourceDefinition>();
         this._cacheVec = new Vector.<ResourceDefinition>();
      }
...
class ResourceDefinition extends EventDispatcher
{
   ...
   public function completeHandler(param1:ContentInfo) : void
   {
      ...
      dispatchEvent(new ResourceQueueEvent("itemComplete"));
   }
