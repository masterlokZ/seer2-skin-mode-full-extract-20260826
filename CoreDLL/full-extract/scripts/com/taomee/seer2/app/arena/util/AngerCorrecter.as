package com.taomee.seer2.app.arena.util
{
   ...
   public class AngerCorrecter
   {
      ...
      {
         var buff:BuffData;
         var push:* = function(f:Function, order:int):void
         {
            fns.push([f,order]);
         };
         var fns:Array = [];
         var args:Array = [0,0,0,0,0];
         for each(buff in pet.buffs)
         {
            if(buff.id == 30006)
            {
               args[0] = buff.count;
            ...
            else if(buff.id == 11900)
            {
               push(function():void
               {
                  pet.anger = Math.min(pet.anger + (Math.min(Math.abs(pet.atk),2) + Math.min(Math.abs(pet.def),2) + Math.min(Math.abs(pet.spa),2) + Math.min(Math.abs(pet.spd),2) + Math.min(Math.abs(pet.spe),2) + Math.min(Math.abs(enemy.atk),2) + Math.min(Math.abs(enemy.def),2) + Math.min(Math.abs(enemy.spa),2) + Math.min(Math.abs(enemy.spd),2) + Math.min(Math.abs(enemy.spe),2)) * 5,100);
               },4);
            }
         }
         push(function():void
