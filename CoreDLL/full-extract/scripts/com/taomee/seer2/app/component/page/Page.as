   ...
   public class Page extends Sprite
   {
      ...
      private function initEventListener() : void
      {
         ...
         if(this._selectable)
         {
            ...
            {
               this._itemVec[_loc1_].addEventListener("click",this.onItemClick);
               _loc1_++;
            }
         }
      }
      ...
      private function onItemClick(param1:MouseEvent) : void
      {
         ...
         dispatchEvent(new Event("selectItemChange"));
      }
      ...
      protected function updateDisplay() : void
      {
         ...
         while(_loc2_ < this._pageCount)
         {
            _loc1_ = this.offset + _loc2_;
            if(_loc1_ < this._dataArr.length)
            {
               this._itemVec[_loc2_].update(this._dataArr[_loc1_]);
            }
            else
            {
               this._itemVec[_loc2_].update(null);
            }
            _loc2_++;
         }
         if(this._selectable)
      ...
      protected function updateSelectedItem() : void
      {
         var _loc1_:int = 0;
         var _loc3_:int = 0;
         var _loc2_:int = this._selectedIndex - this.offset;
         if(_loc2_ >= 0 && _loc2_ < this._pageCount)
         {
            _loc1_ = 0;
            while(_loc1_ < this._itemVec.length)
            {
               this._itemVec[_loc1_].isSelected = _loc2_ == _loc1_;
               _loc1_++;
            }
         }
         else
