package com.taomee.seer2.app.manager
{
   ...
   public class LoginAwardManager
   {
      ...
      private static function onComplete(param1:SceneEvent) : void
      {
         ...
         if(SceneManager.active.type == 1 && SceneManager.prevSceneType != 2)
         {
      ...
      private static function showPhoneGamePanle() : void
      {
         ...
         ServerBufferManager.getServerBuffer(458,function(param1:ServerBuffer):void
         {
      ...
      {
         ServerBufferManager.getServerBuffer(382,function(param1:ServerBuffer):void
         {
            if(param1.readDataAtPostion(1) == 0)
            {
               ModuleManager.toggleModule(URLUtil.getAppModule("JLPanel"),"正在打开...");
               ServerBufferManager.updateServerBuffer(382,1,1);
            }
         });
      }
      ...
      private static function showLoginOnePanel() : void
      {
         var date:Date = null;
         date = new Date(TimeManager.getServerTime() * 1000);
         ServerBufferManager.getServerBuffer(448,function(param1:ServerBuffer):void
         {
            if(param1.readDataAtPostion(date.date) == 0)
            {
               ModuleManager.toggleModule(URLUtil.getAppModule("BroadCastSevenPanel"),"正在打开...");
               ServerBufferManager.updateServerBuffer(448,date.date,1);
            }
         });
      }
      ...
      private static function showLoginReward() : void
      {
         ...
         if(isGrassSignTime())
         {
            DayLimitManager.getDoCount(724,function(param1:int):void
            {
               ...
               if(num == 0)
               {
                  ModuleManager.addEventListener("GoSchoolSignPanel","hide",onGrassSignHide);
      ...
      {
         DayLimitManager.getDoCount(5018,function(param1:int):void
         {
            var num:int = param1;
            ActiveCountManager.requestActiveCountList([250213,250214,205741],function(param1:Parser_1142):void
            {
               if(num <= 0)
               {
      ...
      private static function setSuperPetInitNum() : void
      {
         ...
         ActiveCountManager.requestActiveCount(202957,function(param1:int, param2:int):void
         {
            var type:int = param1;
            var count:int = param2;
            if(count == 0)
            {
               SwapManager.swapItem(1632,1,function(param1:IDataInput):void
               {
                  var data:IDataInput = param1;
                  new SwapInfo(data);
                  SwapManager.swapItem(1622,1,function(param1:IDataInput):void
                  {
                     new SwapInfo(param1);
                     isAskRankSwap = true;
                  });
      ...
      {
         var _loc2_:uint = new Date(2013,0,25,6).getTime() / 1000;
         var _loc1_:uint = new Date(2013,1,21,24).getTime() / 1000;
      ...
      {
         var _loc2_:uint = new Date(2013,10,1,6).getTime() / 1000;
         var _loc1_:uint = new Date(2013,10,21,23).getTime() / 1000;
         if(TimeManager.getServerTime() < _loc1_ && TimeManager.getServerTime() > _loc2_)
         {
            return true;
         }
      ...
      private static function onGrassSignHide(param1:ModuleEvent) : void
      {
         ...
         ModuleManager.removeEventListener("GoSchoolSignPanel","hide",onGrassSignHide);
         ActiveCountManager.requestActiveCount(203133,function(param1:uint, param2:uint):void
         {
            ...
            if(type == 203133 && ActorManager.actorInfo.vipInfo.isRealVip() == false)
            {
               if(count == 1)
               {
                  ModuleManager.addEventListener("VipRechargeBtnPanel","hide",(function():*
                  {
                     var onVipRechargeHide:Function;
                     return onVipRechargeHide = function(param1:ModuleEvent):void
                     {
                        ModuleManager.removeEventListener("VipRechargeBtnPanel","hide",onVipRechargeHide);
      ...
      private static function playAnnouncementTime() : void
      {
         if(isActivityTime())
         {
            ServerBufferManager.getServerBuffer(9,onGetBufferStatus);
         }
      }
      
      public static function isActivityTime() : Boolean
      {
         var _loc2_:uint = new Date(2012,0,19,6).getTime() / 1000;
         var _loc1_:uint = new Date(2012,0,19,24).getTime() / 1000;
         if(TimeManager.getServerTime() < _loc1_ && TimeManager.getServerTime() > _loc2_)
         {
            return true;
         }
      ...
      private static function onGetBufferStatus(param1:ServerBuffer) : void
      {
         ...
         if(_loc2_ == 0)
         {
            ...
            ServerBufferManager.updateServerBuffer(9,1,1);
         }
      }
   }
}
