package com.taomee.seer2.app.manager
{
   ...
   public class MeshBeastReviveManager
   {
      ...
      public var preTimeArr:Array = [0,0,0];
      
      public var shootIndex:int;
      
      private var carriedArr:Vector.<Mobile>;
      
      private var petMap:Array = [90,123,124];
      
      private var pretendPetVec:Vector.<MovieClip> = new Vector.<MovieClip>(3);
      ...
      public function setup() : void
      {
         ...
         if(DateUtil.isInHourScope(13,14,30,30) && DateUtil.isDay(2012,8,17))
         {
            ServerBufferManager.getServerBuffer(39,this.getFollowNum);
      ...
      private function createFollowPet() : void
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < this.followNum)
         {
            _loc1_ = new Mobile();
            _loc1_.resourceUrl = URLUtil.getPetSwf(252);
            this.carriedArr.push(_loc1_);
            ActorManager.getActor().addCarriedMobile(_loc1_,this.startP + 30 * _loc2_);
            _loc2_++;
         }
      }
      ...
      private function initMapPet() : void
      {
         ...
         {
            QueueLoader.load(URLUtil.getQuestAnimation("sceneAnimation/30025"),"swf",this.onResLoaded);
         }
         else
         {
            this.addPetToMap();
      ...
      private function playResult() : void
      {
         ...
         if(resultNum == 0)
         {
            ...
            MovieClipUtil.playMc(this.followPet,1,this.followPet.totalFrames,function():void
            {
               ...
               ++followNum;
               ServerBufferManager.updateServerBuffer(39,1,followNum);
      ...
      private function onResLoaded(param1:ContentInfo) : void
      {
         ...
         while(_loc2_ < this.pretendPetVec.length)
         {
            ...
            this.pretendPetVec[_loc2_].addEventListener("click",this.onTips);
            _loc2_++;
         }
      ...
      {
         NpcManager.addEventListener("actorArrive",this.onQuestDia);
         ...
         this.npc.addEventListener("click",this.showNpcDia);
         if(this.followNum > 0)
         {
      ...
      private function updateCount() : void
      {
         ...
         this.followNum = 0;
         ServerBufferManager.updateServerBuffer(39,0,this.rewardCount);
         ServerBufferManager.updateServerBuffer(39,1,this.followNum);
      }
      ...
      private function getNum() : void
      {
         ...
         if(this.currNum < 10)
         {
            _loc1_ = MobileManager.getMobile(18,"npc");
      ...
      private function onSwapSuccess(param1:IDataInput) : void
      {
         ...
         NpcDialog.show(18,"斯力普",[[0,"放在你的射击工具栏啦，对着荆棘小道，异蘑谷、草系主城场景中可疑的东西射击，就有机会找到神秘草系精灵"]],["好的我去试试看吧！"],[function():void
         {
            ...
            {
               SceneManager.changeScene(1,petMap[int(Math.random() * 3)]);
            }
         }]);
      }
   }
