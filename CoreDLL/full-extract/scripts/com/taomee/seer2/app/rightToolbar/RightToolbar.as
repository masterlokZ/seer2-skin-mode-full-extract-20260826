package com.taomee.seer2.app.rightToolbar
{
   ...
   public class RightToolbar extends Sprite implements IToolBar
   {
      ...
      public var type:String = RightToolbarType.TIME;
      ...
      private var _rightToolBarTime:RightToolBarTime = new RightToolBarTime();
      
      private var _rightToolBarTimeComplete:RightToolBarTimeComplete = new RightToolBarTimeComplete();
      ...
      public function update() : void
      {
         ...
         {
            QueueLoader.load(URLUtil.getRes("loaderLibrary/rightToolbar/" + this._info.sort + ".swf"),"swf",this.onResLoaded);
         }
      }
      ...
      protected function show() : void
      {
         ...
         {
            this.addEventListener("rollOver",this.onOver);
      ...
      private function onOver(param1:MouseEvent) : void
      {
         ...
         var _loc3_:int = 0;
         while(_loc3_ < this.info.toolbarInfoList.length)
         {
            _loc2_ = new RightToolbar();
            if(this.info.toolbarInfoList[_loc3_].classStr != "")
            {
               _loc4_ = getDefinitionByName("com.taomee.seer2.app.rightToolbar.toolbar." + this.info.toolbarInfoList[_loc3_].classStr);
               _loc2_ = new _loc4_() as RightToolbar;
            }
            else
            {
               _loc2_ = new RightToolbar();
            }
            _loc2_.init(this.info.toolbarInfoList[_loc3_]);
            _loc2_.update();
            _loc2_.x = 2;
            _loc2_.y = 60 + _loc3_ * 55;
            addChild(_loc2_);
      ...
      protected function onClick(param1:MouseEvent) : void
      {
         ...
         {
            StatisticsManager.sendNovice("0x10033401");
         }
         else if(this._info.url == "anniversary")
         {
            StatisticsManager.sendNovice("0x10033402");
         }
         else if(this._info.url == "planet")
         {
            StatisticsManager.sendNovice("0x10033403");
         }
         if(this._rightClick == 0)
         {
         }
      ...
      {
         this.removeEventListener("rollOver",this.onOver);
      ...
      public function refreshTime() : void
      {
         ...
         {
            _loc2_ = this._info.lightStartTime.split("_");
            _loc1_ = new Date(_loc2_[0],_loc2_[1] - 1,_loc2_[2],_loc2_[3],_loc2_[4]).getTime() / 1000;
            ...
            else
            {
               ...
               this._timeCount = _loc1_ - TimeManager.getServerTime();
               this.update();
            }
         }
