package com.taomee.seer2.app.actives
{
   ...
   public class DogSipaikeAct
   {
      ...
      public function setup() : void
      {
         ...
         this.buyPassBtn.addEventListener("click",this.toBuy);
         this.toFight = false;
      ...
      public function dispose() : void
      {
         ...
         {
            this.petList[_loc1_].removeEventListener("click",this.subNum);
            _loc1_++;
         }
      }
      ...
      private function createPet() : void
      {
         ...
         {
            _loc1_ = 0;
            while(_loc1_ < this.petNum[_loc2_])
            {
               _loc4_ = 614;
               if(this.LEV_NUM[_loc2_] > 30)
               {
                  _loc4_++;
               }
               _loc3_ = new SpawnedPet(_loc4_,this.FIGHT_ID[_loc2_],this.LEV_NUM[_loc2_]);
               _loc3_.addEventListener("click",this.subNum,false,1000);
               _loc3_.mouseChildren = false;
               _loc3_.name = this.NAME_LIST[_loc2_];
               MobileManager.addMobile(_loc3_,"spawnedPet");
               this.petList.push(_loc3_);
               _loc1_++;
            }
            _loc2_++;
         }
