   ...
   public class ShopManager
   {
      ...
      private static function processNextBuyRequest() : void
      {
         ...
         if(Boolean(_buyWaitList.length) && _isBuyRequesting == false)
         {
            ...
            _loc2_ = _buyWaitList.shift();
            _obj = _loc2_;
            _buySuccessCallBack = _loc2_.success;
            _buyErrorCallBack = _loc2_.error;
            _tryId = MatchingConfig.getOldId(_loc2_.id);
            _count = _loc2_.count;
            ...
            Connection.send(CommandSet.MI_BUY_ITEM_1224,_loc2_.id,_loc2_.count,_loc2_.by);
            _loc1_ = MatchingConfig.getOldId(_loc2_.id);
            MapPanelProtocolPanel.instance().buyID = _loc1_;
            MapPanelProtocolPanel.instance().addLog(6,"\n购买协议：" + CommandSet.MI_BUY_ITEM_1224 + "\n\t newID=" + _loc1_ + " oldID=" + _loc2_.id + " num=" + _loc2_.count + " price=" + ShopPanelConfig.getItemPrice(_loc1_) * _loc2_.count + "星钻");
         }
      }
