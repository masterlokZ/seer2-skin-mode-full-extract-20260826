package com.taomee.seer2.app.entity
{
   ...
   public class Mineport extends AnimateElement implements IExtendedEntity
   {
      ...
      public function setData(param1:XML) : void
      {
         ...
         var _loc3_:String = param1.attribute("resId").toString();
         this.resourceUrl = URLUtil.getMapMineSwf(_loc3_);
         this.name = param1.attribute("name").toString();
         TooltipManager.addCommonTip(this,this.name);
         var _loc5_:String = param1.attribute("pos").toString();
         _loc4_ = Util.parsePositionStr(_loc5_);
         this.x = _loc4_.x;
         this.y = _loc4_.y;
         var _loc2_:String = param1.attribute("actorPos").toString();
         this._actorPosition = Util.parsePositionStr(_loc2_);
      ...
      private function addPortClickEventListener() : void
      {
         ...
         addEventListener("click",this.onPortClick);
      }
      ...
      private function onPortClick(param1:MouseEvent) : void
      {
         ...
         var _loc4_:Actor = ActorManager.getActor();
         _loc3_ = _loc4_.isArrivedPosition(this._actorPosition);
         if(_loc3_)
         {
            this.askMine();
         }
         ...
         {
            _loc4_.addEventListener("finished",this.onActorMoveFinished);
            _loc4_.runToLocation(this._actorPosition.x,this._actorPosition.y);
         }
      ...
      private function onActorMoveFinished(param1:MoveEvent) : void
      {
         ...
         var _loc3_:Boolean = _loc2_.isArrivedPosition(this._actorPosition);
         if(_loc3_ == true)
         {
            this.askMine();
         }
      ...
      private function onEnter(param1:Event) : void
      {
         ...
         {
            this._mineAnimation.removeEventListener("enterFrame",this.onEnter);
            LayerManager.resetOperation();
            ...
            InteractiveRewardManager.requestReward(this._serverConfigId,function(param1:Parser_1060):void
            {
               param1.showResult();
               addDiggedNum();
               ItemManager.dispatchEvent("mineSuccess",_serverConfigId);
            },this.onDigError);
         }
      }
      ...
      private function askMine() : void
      {
         ...
         this._mineAnimation.addEventListener("enterFrame",this.onEnter);
      ...
      private function getAvailableDigTime() : int
      {
         ...
         var _loc2_:SharedObject = SharedObjectManager.getUserSharedObject("digging");
         var _loc1_:String = this.generateDateKey();
         var _loc4_:String = this.generateMineKey();
         if(_loc2_.data[_loc1_] == null)
         {
            this.resetSharedObject(_loc2_,_loc1_);
         }
         _loc3_ = _loc2_.data[_loc1_][_loc4_];
         if(_loc3_ == null)
         {
            _loc3_ = {};
            _loc3_.diggedNum = 0;
            _loc2_.data[_loc1_][_loc4_] = _loc3_;
            SharedObjectManager.flush(_loc2_);
         }
         return this._limit - _loc3_.diggedNum;
      }
      ...
      override public function dispose() : void
      {
         ...
         removeEventListener("click",this.onPortClick);
         super.dispose();
      }
   }
}
