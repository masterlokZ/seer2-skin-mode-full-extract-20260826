   ...
   public class PVPDoorSupport implements ISupportable, IDoorSupport
   {
      ...
      private function initUI() : void
      {
         ...
         onGetUserSimpleInfo = function(param1:MessageEvent):void
         {
            ...
            var _loc3_:ByteArray = param1.message.getRawData();
            _loc3_.position = 0;
            var _loc5_:uint = _loc3_.readUnsignedInt();
            if(_loc5_ == opponentId)
            {
               Connection.removeCommandListener(CommandSet.USER_GET_SIMPLE_INFO_1028,onGetUserSimpleInfo);
               _loc4_ = new UserInfo();
               _loc4_.id = _loc5_;
               UserInfoParseHelper.readSimpleInfo(_loc4_,_loc3_);
               _initData["remoteUserInfo"] = _loc4_;
               _loc2_ = ActorManager.getActor();
               _loc2_.direction = Direction.RIGHT_DOWN;
               _loc2_.x = PVPDoorUtil.getSelfPosition(_supportInfo.doorRule).x;
               _loc2_.y = PVPDoorUtil.getSelfPosition(_supportInfo.doorRule).y;
