package com.taomee.seer2.app.gameRule.behavior
{
   ...
   public class InvitePetSelectBehavior
   {
      ...
      public function onInviteFighter(param1:Vector.<uint>) : void
      {
         ...
         this.addListeners();
         var _loc5_:uint = this._inviteUser.id;
         var _loc7_:LittleEndianByteArray = new LittleEndianByteArray();
         _loc7_.writeUnsignedInt(_loc5_);
         _loc7_.writeUnsignedInt(this._fightMode);
         _loc7_.writeByte(this._fightType);
         _loc7_.writeUnsignedInt(param1.length);
         var _loc6_:uint = 0;
         while(_loc6_ < param1.length)
         {
            _loc7_.writeUnsignedInt(param1[_loc6_]);
            _loc6_++;
         }
         Connection.send(CommandSet.FIGHT_INVITE_PVP_1512,_loc7_);
         var _loc3_:Array = [];
         for each(_loc2_ in param1)
         {
            _loc3_.push(_loc2_);
         }
         _loc4_ = SOMultiBehavior.getKey(this._fightMode);
         SOMultiBehavior.writeDefaultPets(_loc4_,_loc3_);
      }
