package com.taomee.seer2.app.net.parser
{
   ...
   public class Parser_1144
   {
      ...
      {
         var _loc5_:uint = this.addItemVec.length;
         var _loc4_:Vector.<ItemDescription> = new Vector.<ItemDescription>();
         var _loc3_:uint = 0;
         while(_loc3_ < _loc5_)
         {
            if(this.addItemVec[_loc3_].id == 1)
            {
               ActorManager.actorInfo.coins += this.addItemVec[_loc3_].count;
               if(param1)
               {
                  _loc4_.push(new ItemDescription(this.addItemVec[_loc3_].id,this.addItemVec[_loc3_].count));
               }
            }
            else
            {
