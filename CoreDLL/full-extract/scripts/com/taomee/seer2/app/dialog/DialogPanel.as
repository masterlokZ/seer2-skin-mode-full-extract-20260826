package com.taomee.seer2.app.dialog
{
   ...
   public class DialogPanel extends Sprite
   {
      ...
      {
         instance.addEventListener("close",param1);
      }
      
      public static function removeCloseEventListener(param1:Function) : void
      {
         instance.removeEventListener("close",param1);
      }
      ...
      {
         this._dialogBox.addEventListener("customReplayClick",this.onCustomReplayClick);
         this._dialogBox.addEventListener("updateContent",this.onupdate);
      }
      
      private function onCustomReplayClick(param1:DialogBoxEvent) : void
      {
         dispatchCustomEvent("customReplyClick",param1.content);
      }
      ...
      {
         this._functionalityBox.addEventListener("questClick",this.onQuestUnitClick);
         this._functionalityBox.addEventListener("customUnitClick",this.onCustomUnitClick);
         this._functionalityBox.addEventListener("questNewClick",this.onQuestUnitNewClick);
      }
      ...
      private function onQuestUnitClick(param1:FunctionalityBoxEvent) : void
      {
         ...
         var _loc4_:int = int(param1.content.stepId);
         _loc3_ = _loc2_.getQuestDialogDefintion(_loc4_);
         if(_loc3_)
         {
            this._dialogBox.setQuestInfo(_loc2_,_loc4_,_loc3_);
            if(instance._currentNpcId != this._prevNpc)
            {
               this.tweenDialog();
               this.dialogPanel = this;
         ...
         dispatchCustomEvent("questUnitClick",{
            "questId":_loc2_.id,
            "stepId":_loc4_
         });
      }
      ...
      private function onQuestUnitNewClick(param1:FunctionalityBoxEvent) : void
      {
         ...
         this.hide("");
         dispatchCustomEvent("npcQuestClick",{
            "questId":_loc2_,
            "stepId":_loc3_
         });
      }
      ...
      private function onCustomUnitClick(param1:FunctionalityBoxEvent) : void
      {
         ...
         this.hide("");
         dispatchCustomEvent("customUnitClick",param1.content);
      }
      ...
      private function addQuestFunctionalityUnit() : void
      {
         ...
         {
            if(_loc1_.getCurrentNpcIdVec().indexOf(this._currentNpcId) != -1)
            {
               if(_loc1_.id != 10072 && _loc1_.id != 10073 && _loc1_.id != 10074)
               {
                  this._functionalityBox.createQuestUnit(this._currentNpcId,_loc1_);
               }
               else
               {
      ...
      private function afterShowDialog() : void
      {
         ...
         if(this._currentNpcId != 0)
         {
            dispatchCustomEvent("dialogShow",this._currentNpcId);
         }
      }
      ...
      private function afterHideDialog(param1:String) : void
      {
         ...
         {
            dispatchCustomEvent("dialogClosed",new DialogPanelEventData("close",param1));
            if(hasEventListener("close"))
            {
               dispatchEvent(new DialogCloseEvent(param1,"close"));
            }
         }
      }
      ...
      private function setDialogBoxXml(param1:XML) : void
      {
         ...
         if(this._currentDialog.npcName == "")
         {
