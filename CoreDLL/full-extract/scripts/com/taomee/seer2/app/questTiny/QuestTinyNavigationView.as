package com.taomee.seer2.app.questTiny
{
   ...
   public class QuestTinyNavigationView extends Sprite
   {
      ...
      private const typeArr:Array = ["recommendQuest","mainQuest","starQuest"];
      ...
      public function QuestTinyNavigationView(param1:MovieClip)
      {
         ...
         this.createScroller();
         QuestManager.addEventListener("accept",this.onQuestRefresh);
         QuestManager.addEventListener("init",this.onQuestRefresh);
         QuestManager.addEventListener("stepUpdateBuffer",this.onQuestRefresh);
         QuestManager.addEventListener("complete",this.onQuestRefresh);
         QuestManager.addEventListener("stepComplete",this.onQuestRefresh);
      }
      ...
      private function setupCategoryItem() : void
      {
         ...
         while(_loc1_ < this.typeArr.length)
         {
            ...
            this._categoryBtns[_loc1_].addEventListener("click",this.onCategory);
            _loc1_++;
         }
      }
      ...
      private function createNavigationItem() : void
      {
         ...
         while(_loc1_ < this.typeArr.length)
         {
            ...
            this._navigationItems[_loc1_].addEventListener("layoutComplete",this.onLayoutComplete);
            _loc1_++;
         }
         this._itemRect = new Rectangle(0,30,213,181);
