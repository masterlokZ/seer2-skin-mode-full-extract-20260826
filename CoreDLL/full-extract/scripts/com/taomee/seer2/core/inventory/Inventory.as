   ...
   public class Inventory extends EventDispatcher
   {
      ...
      protected function addStackableItem(param1:StackableItem) : void
      {
         ...
         if(this.containsUniqueId(param1.uniqueId))
         {
            ...
            if(_loc3_ > param1.quantityLimit)
            {
               this.dispatchInventoryErrorEvent("quantityLimitReach");
               return;
            }
            _loc2_.quantity = _loc3_;
            this.dispatchInventoryEvent("quantityAdd",param1.referenceId);
            this.dispatchInventoryEvent("updateQuantity",param1.referenceId);
         }
         else
         {
            if(param1.getOccupiedSpace() > this.getAvailableCapacity())
            {
               this.dispatchInventoryErrorEvent("full");
               return;
            }
            this._content.add(param1.uniqueId,param1);
            this.dispatchInventoryEvent("quantityAdd",param1.referenceId);
            this.dispatchInventoryEvent("add",param1.referenceId);
            this.dispatchInventoryEvent("updateQuantity",param1.referenceId);
         }
      ...
      protected function updateUsedCapacity() : void
      {
         ...
         var _loc2_:Array = this._content.getValues();
         for each(_loc1_ in _loc2_)
         {
            this._usedCapacity += _loc1_.getOccupiedSpace();
         }
      ...
      public function removeItemByUniqueId(param1:int) : void
      {
         ...
         if(this._content.containsKey(param1))
         {
            ...
            this.dispatchInventoryEvent("remove",_loc2_.referenceId);
            this.dispatchInventoryEvent("updateQuantity",_loc2_.referenceId);
         }
         else
         {
            this.dispatchInventoryErrorEvent("noSuchItem");
         }
      }
      ...
      public function reduceItemQuantity(param1:int, param2:int) : void
      {
         ...
         if(this.containsReferenceId(param1))
         {
            ...
            else
            {
               ...
               this.dispatchInventoryEvent("updateQuantity",_loc3_.referenceId);
            }
         }
         else
         {
