package com.taomee.seer2.core.map
{
   ...
   public class ResContentLibrary
   {
      ...
      public static function updateRes() : void
      {
         ...
         var _loc1_:int = 0;
         while(_loc1_ < resList.length)
         {
            _loc2_ = resList[_loc1_];
         ...
         _loc1_ = 0;
         while(_loc1_ < scaleList.length)
         {
            _loc2_ = scaleList[_loc1_];
            if(Boolean(_loc2_) && Boolean(_loc2_.parent))
            {
               _loc2_.scaleX = LayerManager.stage.stageWidth / 1200;
               _loc2_.scaleY = LayerManager.stage.stageHeight / 660;
            }
            _loc1_++;
         }
         _loc1_ = 0;
         while(_loc1_ < initList.length)
         {
            _loc2_ = initList[_loc1_];
            if(Boolean(_loc2_) && Boolean(_loc2_.parent))
            {
               _loc2_.x = (LayerManager.stage.stageWidth - 1200) / 2;
               _loc2_.y = (LayerManager.stage.stageHeight - 660) / 2;
            }
            _loc1_++;
         }
         _loc1_ = 0;
         while(_loc1_ < initYList.length)
         {
            _loc2_ = initYList[_loc1_];
            if(Boolean(_loc2_) && Boolean(_loc2_.parent))
            {
               _loc2_.y = (LayerManager.stage.stageHeight - 660) / 2;
            }
