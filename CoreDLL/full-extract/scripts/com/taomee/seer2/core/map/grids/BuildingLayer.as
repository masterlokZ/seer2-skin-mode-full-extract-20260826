package com.taomee.seer2.core.map.grids
{
   import flash.display.BitmapData;
   
   public class BuildingLayer extends BaseDisplayObject
   {
      
      public var _buildingArray:Array;
      
      private var _maxNum:int = 0;
      
      private var _map:GameMap;
      
      private var _imageDataMap:HashMap;
      
      public function BuildingLayer(param1:GameMap)
      {
         super();
         this._map = param1;
         this._buildingArray = [];
         this._imageDataMap = new HashMap();
      }
      
      public function drawByXml(param1:XML) : void
      {
         var _loc2_:XML = null;
         var _loc4_:GameMapItem = null;
         var _loc3_:BitmapData = null;
         for each(_loc2_ in param1.items.item)
         {
            _loc4_ = new GameMapItem();
            _loc4_.parentLayer = this;
            _loc4_.x = _loc2_.@px;
            _loc4_.y = _loc2_.@py;
            _loc3_ = BitmapData(this._imageDataMap.getValue(_loc2_.@file[0]));
            if(_loc3_ != null)
            {
               _loc4_.reset(_loc3_,_loc2_);
            }
            else
            {
               _loc4_.configXml = _loc2_;
               _loc4_.loadImage();
            }
            this._buildingArray.push(_loc4_);
            this.addChild(_loc4_);
         }
      }
      
      public function get imageDataMap() : HashMap
      {
         return this._imageDataMap;
      }
      
      public function get buildingArray() : Array
      {
         return this._buildingArray;
      }
      
      public function dispose() : void
      {
         this._imageDataMap.clear();
         this._buildingArray.length = 0;
         this._buildingArray = null;
      }
   }
}

