   ...
   public class Proclaim extends Sprite
   {
      ...
      private function next() : void
      {
         ...
         if(this._txt.text.indexOf("开启幸运礼包获得了至尊资质果实") != -1)
         {
            ...
            this.addEventListener("click",openVipPanel);
         ...
         this.removeEventListener("click",openVipPanel);
         if(this._txt.text.indexOf("在乐园金字塔中抽取了终极大奖") != -1)
         {
            this.mouseEnabled = true;
            this.buttonMode = true;
            this.addEventListener("click",this.openVipPyramid);
         }
         else
         {
            this.mouseEnabled = false;
            this.buttonMode = false;
            this.removeEventListener("click",this.openVipPyramid);
         }
         if(this._txt.text.indexOf("谁能驾驭摩托驰骋黑夜") != -1)
         {
            this.mouseEnabled = true;
            this.buttonMode = true;
            this.addEventListener("click",openVipTurn);
         }
         else
         {
            this.mouseEnabled = false;
            this.buttonMode = false;
            this.removeEventListener("click",openVipTurn);
         }
         this._isPlaying = true;
         this._txt.x = this._background.width + 50;
      }
